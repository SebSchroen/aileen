require(['bowser', 'underscore', 'backbone', 'semver', 'react', 'react-dom', 'html-react-parser', 'libraries/mediaelement-and-player', 'libraries/SCORM_API_wrapper', 'libraries/lzma-min', 'libraries/lzma_worker-min.js', 'libraries/js-cookie.js', 'libraries/jquery.keycombo'], (function (Bowser, _$1, Backbone$1, semver, React, ReactDOM, HTMLReactParser, mediaelementAndPlayer, pipwerks, lzmaMin, lzma_workerMin_js, Cookies, jquery_keycombo) { 'use strict';
  ...
  class OfflineStorage$1 extends Backbone.Controller {
    initialize(handler) {
      this.ready = true;
  ...
  const offlineStorage = new OfflineStorage$1();
  ...
  class Device extends Backbone.Controller {
    ...
    get screenSizes() {
      ...
      for (const [name, value] of screenSizesList) {
        screenSizes[name] = value > screensizeEmThreshold ? value / baseFontSize : value;
      }
      return screenSizes;
    }
  ...
  var AriaDisabled = __AMD("core/js/a11y/ariaDisabled", class BrowserFocus extends Backbone.Controller {
    initialize({
      a11y
    }) {
      this.a11y = a11y;
    ...
    isAriaDisabled($element) {
      ...
      const isAriaDisabled = $element.closest('[aria-disabled=true]').length === 1 || $closestFor.length && $(`#${$closestFor.attr('for')}`).is('[aria-disabled=true]');
  ...
  var BrowserConfig = __AMD("core/js/a11y/browserConfig", class BrowserConfig extends Backbone.Controller {
    initialize({
      a11y
    }) {
      this.a11y = a11y;
  ...
  var Scroll = __AMD("core/js/a11y/scroll", class Scroll extends Backbone.Controller {
    ...
    _onKeyDown(event) {
      ...
      if (this._a11y.isPopupOpen && !this._isScrollable($(event.target))) {
        ...
        const $firstScrollable = this._isScrollable($openPopup) ? $openPopup : this._a11y._findFirstForwardDescendant($openPopup, this._isScrollable);
    ...
    _getScrollDelta(event) {
      ...
      if (isTouchEvent) {
        ...
        deltaY = [33, 36, 38].includes(event.which) ? 1 : [34, 35, 40].includes(event.which) ? -1 : 0;
      } else {
  ...
  var TopOfContentObject = __AMD("core/js/a11y/topOfContentObject", class TopOfContentObject extends Backbone.Controller {
    initialize({
      a11y
    }) {
      this.a11y = a11y;
    ...
    updateElement(location) {
      ...
      json._globals = Adapt$1.course.get('_globals');
  ...
  var Popup = __AMD("core/js/a11y/popup", class Popup extends Backbone.Controller {
    ...
    _addPopupLayer($popupElement) {
      ...
      logging$1.deprecated('a11y/popup opened: Use native dialog tag for', $popupElement);
  ...
  class Data extends AdaptCollection {
    ...
    async init() {
      ...
      Adapt$1.build = new BuildModel(null, {
        url: `adapt/js/build.min.js?timestamp=${Date.now()}`,
        reset: true
      });
    ...
    loadConfigData() {
      Adapt$1.config = new ConfigModel(null, {
        url: Adapt$1.build.get('coursedir') + '/config.' + Adapt$1.build.get('jsonext') + `?timestamp=${Adapt$1.build.get('timestamp')}`,
        reset: true
      });
    ...
    async loadManifestFiles(languagePath) {
      ...
      try {
        allFileData = await Promise.all(manifest.map(filePath => {
          return this.getJSON(`${languagePath}${filePath}?timestamp=${Adapt$1.build.get('timestamp')}`);
        }));
      } catch (error) {
      ...
      allModelData.forEach(modelData => {
        ...
        try {
          ...
          logging$1.error(`Failed to load object ${modelData.__path__}${Object.hasOwn(modelData, '__index__') ? `[${modelData.__index__}]` : ''}`);
          logging$1.error(error);
          return;
        }
        this.push(modelData);
    ...
    async triggerDataReady(newLanguage, previousLanguage) {
      if (newLanguage) {
        Adapt$1.trigger('app:languageChanged', newLanguage, previousLanguage);
        await wait$1.queue();
      }
    ...
    findById(id) {
      ...
      if (!model) {
        logging$1.warn(`data.findById() unable to find id: ${id}`);
        return;
      }
      return model;
    }
    findViewByModelId(id) {
      ...
      if (!model || !Adapt$1.parentView) return;
      const idPathToView = [id];
      const currentLocationId = location$1._currentId;
      ...
      if (!currentLocationModel) {
        return logging$1.warn(`data.findViewByModelId() unable to find view for model id: ${id}`);
      }
    ...
    findByTrackingPosition(trackingPosition) {
      ...
      if (!trackingIdModel) {
        logging$1.warn(`data.findByTrackingPosition() unable to find trackingPosition: ${trackingPosition}`);
        return;
      }
    ...
    checkTrackingIds() {
      ...
      const duplicateTrackingIds = Object.entries(trackingIdCounts).filter(([id, group]) => group.length > 1).map(([id, group]) => `${id}:[${group.map(item => item._id).join(', ')}]`);
  ...
  class A11y extends Backbone.Controller {
    ...
    initialize() {
      ...
      this.config = null;
      this._ariaDisabled = new AriaDisabled({
        a11y: this
      });
      this._browserConfig = new BrowserConfig({
        a11y: this
      });
      ...
      this._topOfContentObject = new TopOfContentObject({
        a11y: this
      });
    ...
    _onNavigationEnd(view) {
      if (view?.model?.get('_id') !== location$1._currentId || !this.isEnabled()) {
        return;
      }
    ...
    isReadable($element, checkParents = true) {
      ...
      const isNotVisible = $branch.toArray().some(item => {
        ...
        return style.display === 'none' || style.visibility === 'hidden' || item.getAttribute('aria-hidden') === 'true';
      });
      if (isNotVisible) {
        return false;
      }
  ...
  async function transitionNextFrame() {
    return new Promise(resolve => requestAnimationFrame(resolve));
  }
  async function transitionsEnded($element, property = null) {
    $element = $($element).first();
    ...
    return new Promise(resolve => {
      ...
      const done = didTransition => {
        ...
        $element.off('transitionrun', onRun).off('transitioncancel transitionend', onEnd);
        resolve(didTransition);
      };
      $element.on('transitionrun', onRun).on('transitioncancel transitionend', onEnd);
      const resolveInterval = setTimeout(() => {
        logging$1.warn('transition could/did not run, forcing resolution', $element[0]);
        if (hasActiveTransition($element, property)) return;
        done(false);
      }, timeoutDuration);
    });
  ...
  function getTransitionsLongestEndTime($element, property = null) {
    ...
    const longestEndTime = property && !isAllProperty ? endTimes[property] : Math.max(...Object.values(endTimes));
    return (longestEndTime ?? 0) * 1000;
  }
  ...
  function hasActiveTransition($element, property = null) {
    ...
    if (property) transitions = transitions.filter(({
      transitionProperty
    }) => transitionProperty === property);
    return Boolean(transitions.length);
  }
  __AMD("core/js/transitions", {
    transitionsEnded,
  ...
  class ShadowView extends Backbone__default["default"].View {
    className() {
  ...
  var ShadowView$1 = __AMD("core/js/views/ShadowView", ShadowView);

  class Shadow extends Backbone__default["default"].Controller {
    initialize() {
      this.listenTo(Adapt$1, {
    ...
    onAdaptStart() {
      this._shadowView = new ShadowView$1();
  ...
  var shadow = __AMD("core/js/shadow", new Shadow());
  ...
  class DrawerView extends Backbone__default["default"].View {
    tagName() {
      return 'dialog';
    }
    className() {
      return ['drawer', 'u-display-none'].filter(Boolean).join(' ');
    }
    attributes() {
      return {
        'aria-modal': 'true',
    ...
    initialize() {
      ...
      if (showEasing.includes('elastic')) {
        logging$1.removed('drawer show elastic easing is replaced with quint');
        showEasing = showEasing.replace('elastic', 'quint');
      }
      if (showEasing.includes('bounce')) {
        logging$1.removed('drawer show bounce easing is replaced with back');
        showEasing = showEasing.replace('bounce', 'back');
      }
      if (hideEasing.includes('elastic')) {
        logging$1.removed('drawer hide elastic easing is replaced with quint');
        hideEasing = hideEasing.replace('elastic', 'quint');
      }
      if (hideEasing.includes('bounce')) {
        logging$1.removed('drawer hide bounce easing is replaced with back');
    ...
    get config() {
      return Adapt$1.config.get('_drawer');
    }
    setupEventListeners() {
      this.onKeyDown = this.onKeyDown.bind(this);
      $(window).on('keydown', this.onKeyDown);
      this.el.addEventListener('mousedown', this.onShadowClicked, {
        capture: true
      });
    }
    onKeyDown(event) {
    ...
    setDrawerPosition(position) {
      ...
      const isRTL = Adapt$1.config.get('_defaultDirection') === 'rtl';
      ...
      this.$el.removeClass(`is-position-${this.drawerPosition}`).addClass(`is-position-${position}`);
    ...
    openCustomView(view, hasBackButton = true, position) {
      ...
      this.$('.drawer__holder').html(view instanceof Backbone__default["default"].View ? view.$el : view);
    }
    ...
    async showDrawer(emptyDrawer, position = null) {
      ...
      this.$el.removeClass('u-display-none').attr('aria-hidden', 'false');
      if (!this._isVisible) {
    ...
    async hideDrawer($toElement, {
      ...
      this.$el.removeAttr('style').addClass('u-display-none').attr('aria-hidden', 'true');
      this.setDrawerPosition(this._globalDrawerPosition);
    }
    remove() {
      this.hideDrawer(null, {
        force: true
      });
      super.remove();
      this.el.removeEventListener('mousedown', this.onShadowClicked, {
        capture: true
      });
  ...
  var TemplateRenderEvent = __AMD("core/js/templateRenderEvent", class TemplateRenderEvent {
  ...
  (function () {
    ...
    React__default["default"].createElement = (...args) => {
      const name = args[0];
  ...
  function prefixClasses(prefixes, ...args) {
  ...
  const SECOND_PASS = 2;
  const THIRD_PASS = 3;
  var TooltipItemView = __AMD("core/js/views/TooltipItemView", class TooltipItemView extends Backbone.View {
    className() {
      return ['tooltip', this.model.get('isTargetFixedPosition') && 'is-fixed', this.isStatic && 'is-static', this.model.get('tooltipClasses') || 'is-vertical-axis is-arrow-middle is-bottom is-middle', this.model.get('isShown') && 'is-shown', this.model.get('wasShown') && 'was-shown', this.model.get('_classes')].filter(Boolean).join(' ');
    }
    attributes() {
      return {
    ...
    initialize({
      ...
      this.listenTo(Adapt$1.parentView, 'preRemove', this.remove);
      if (!this.isStatic) {
    ...
    get environment() {
      const navigationAlignment = Adapt$1.course.get('_navigation')?._navigationAlignment ?? 'top';
      ...
      return {
        ...
        isRTL: Adapt$1.config.get('_defaultDirection') === 'rtl',
        topNavOffset: navigationAlignment === 'top' ? navHeight : 0,
        bottomNavOffset: navigationAlignment === 'bottom' ? navHeight : 0,
        targetDOMRect: this.isStatic ? this.applyStaticScrollOffset(targetDOMRect) : targetDOMRect,
        clientDOMRect: {
          width: parseInt(getComputedStyle(document.body).width),
          height: this.isStatic ? parseInt(getComputedStyle(document.getElementById('app')).height) : $('html')[0].clientHeight
        },
        tooltipDOMRect: this.isStatic ? this.applyStaticScrollOffset(tooltipDOMRect) : tooltipDOMRect,
        arrowDOMRect: this.isStatic ? this.applyStaticScrollOffset(arrowDOMReact) : arrowDOMReact,
    ...
    render() {
      ...
      ReactDOM__default["default"].render(React__default["default"].createElement(Template, this.model.toJSON()), this.el);
    }
    changed() {
      if (!this.model) return;
  ...
  function fetchCSSVariables(target, {
    offset,
    distance,
    hasArrow
  }) {
  ...
  function parseRelativePosition({
    ...
    [positions, isOutside, isInside, isAreaAuto] = fixConditions(positions, 'outside', 'inside', 'area');
    if (isAreaAuto) positions.unshift('area');
    [positions, isArrowStart, isArrowEnd, isArrowMiddle, hasArrowSpecified] = fixConditions(positions, 'start', 'end', 'arrow');
    [positions, isTop, isBottom, isCenter, hasVerticalSpecified, verticalIndex] = fixConditions(positions, 'top', 'bottom', 'vertical');
    [positions, isLeft, isRight, isMiddle, hasHorizontalSpecified, horizontalIndex] = fixConditions(positions, 'left', 'right', 'horizontal');
    ...
    let specifiedCount = [hasArrowSpecified, hasHorizontalSpecified, hasVerticalSpecified].reduce((sum, bool) => sum + (bool ? 1 : 0), 0);
    ...
    while (true) {
      ...
      if (shouldHorizontal) {
        ...
        if (isAuto) wasHorizontalFilled = true;else hasHorizontalSpecified = true;
        horizontalIndex = indexOf;
      } else if (shouldArrow) {
        positions[indexOf] = 'arrow';
        if (isAuto) wasArrowFilled = true;else hasArrowSpecified = true;
      } else if (shouldVertical) {
        positions[indexOf] = 'vertical';
        if (isAuto) wasVerticalFilled = true;else hasVerticalSpecified = true;
    ...
    if (isOutside && isVerticalAxis && isCenter) {
      isTop = true;
      isCenter = false;
      logging$1.info(`tooltips: "${position}"(${positions.join(' ')}) is not a valid location, assuming side 'top'`);
    } else if (isOutside && isHorizontalAxis && isMiddle) {
      isLeft = true;
      isMiddle = false;
      logging$1.info(`tooltips: "${position}"(${positions.join(' ')}) is not a valid location, assuming side 'left'`);
    }
  ...
  function convertToDistanceRect(DOMRect, clientDOMRect) {
    return {
      top: DOMRect.top,
      left: DOMRect.left,
  ...
  function constrainDimensions(distanceRect, viewPortPadding, isTargetFixedPosition, topNavOffset, bottomNavOffset) {
    return {
  ...
  function position({
    ...
    const constrainedClientDistRect = {
      ...
      height: clientDOMRect.height - viewPortPadding * 2 - (isTargetFixedPosition ? 0 : topNavOffset + bottomNavOffset)
    };
    let {
      isOutside,
      isInside,
    ...
    if (pass >= SECOND_PASS) {
      ...
      const overallHeight = isOutside && isVerticalAxis ? constrainedTooltipDistRect.height + constrainedArrowDistRect.height + distance : constrainedTooltipDistRect.height;
      const overallWidth = isOutside && isHorizontalAxis ? constrainedTooltipDistRect.width + constrainedArrowDistRect.width + distance : constrainedTooltipDistRect.width;
      ...
      const canFitWidthLength = isVerticalAxis ? overallWidth <= constrainedClientDistRect.width : overallWidth <= constrainedTargetDistRect.left || overallWidth <= constrainedTargetDistRect.right;
      const canFitHeightLength = isHorizontalAxis ? overallHeight <= constrainedClientDistRect.height : overallHeight <= constrainedTargetDistRect.top || overallHeight <= constrainedTargetDistRect.bottom;
      ...
      const isArrowOffscreen = isVerticalAxis && (constrainedArrowDistRect.left < offset || constrainedArrowDistRect.right < offset) || isHorizontalAxis && (constrainedArrowDistRect.top < offset || constrainedArrowDistRect.bottom < offset);
      const isBadShape = !canFitHeightLength || !canFitWidthLength;
      const isSwapAxis = isArrowOffscreen && isHorizontalAxis && isOverflowVertical || isArrowOffscreen && isVerticalAxis && isOverflowHorizontal || isHorizontalAxis && !canFitInHorizontalArea && isVerticalAreaLarger || isVerticalAxis && !canFitInVerticalArea && isHorizontalAreaLarger;
      const isFillArea = isSwapAxis && isBadShape || isVerticalAxis && isOverflow && isBadShape && (canFitInVerticalArea || isVerticalAreaLarger) || isHorizontalAxis && isOverflow && isBadShape && (canFitInHorizontalArea || isHorizontalAreaLarger);
      if (pass === SECOND_PASS && (isBadShape || isOverflow)) {
        ...
        const isSwapVerticalSide = !(isHorizontalAxis && canFitInHorizontalArea) && (isFillArea && !(isTop && canFitInTopArea || isBottom && canFitInBottomArea) || !isFillArea && (isBottom && isOverflowBottom || isTop && isOverflowTop));
        ...
        const isSwapHorizontalSide = !(isVerticalAxis && canFitInVerticalArea) && (isFillArea && !(isLeft && canFitInLeftArea || isRight && canFitInRightArea) || !isFillArea && (isLeft && isOverflowLeft || isRight && isOverflowRight));
    ...
    const tooltipClasses = [isOutside && 'is-outside', isInside && 'is-inside', isArrowSnap && 'is-arrow-snap', isVerticalAxis && 'is-vertical-axis', isHorizontalAxis && 'is-horizontal-axis', isLeft && 'is-left', isMiddle && 'is-middle', isRight && 'is-right', isFillWidth && 'is-fill-width', isSnapLeft && 'is-snap-left', isSnapRight && 'is-snap-right', isTop && 'is-top', isCenter && 'is-center', isBottom && 'is-bottom', isFillHeight && 'is-fill-height', isSnapTop && 'is-snap-top', isSnapBottom && 'is-snap-bottom', hasArrow && 'has-arrow', isArrowStart && 'is-arrow-start', isArrowMiddle && 'is-arrow-middle', isArrowEnd && 'is-arrow-end'].filter(Boolean).join(' ');
    const {
      scrollOffsetLeft,
      scrollOffsetTop
  ...
  var TooltipItemModel = __AMD("core/js/models/TooltipItemModel", class TooltipItemModel extends Backbone__default["default"].Model {
    defaults() {
      return {
        ...(Adapt$1.course.get('_tooltips') || {}),
        _id: null,
        _isEnabled: true,
        _classes: '',
        disabledText: '{{text}}',
  ...
  class DOMElementModificationEventObject {
    constructor({
      type,
      target,
      changedAttributes,
  ...
  class DOMElementModifications extends Backbone__default["default"].View {
    initialize({
    ...
    static calculateEventNames(eventNames) {
      ...
      const events = _.uniq([isNew && 'added', eventNames[eventNames.length - 1]].filter(Boolean));
      return events;
    }
    _onMutation(list) {
      const reducedChanges = list.reduce((changes, item) => {
        ...
        const targetNodes = isChangedEvent ? [item.target] : Array.from(isAddedEvent ? addedNodes : removedNodes).flatMap(node => {
          return this._watch.subtree ? [node, ...node.querySelectorAll('*')] : [node];
        });
        const eventName = isAddedEvent ? 'added' : isRemovedEvent ? 'removed' : 'changed';
        targetNodes.forEach(target => {
          const value = changes.get(target) ?? {
            eventNames: [],
            previousAttributes: this._watch.attributes ? {} : null
          };
      ...
      for (let [target, {
        ...
        eventNames.forEach(eventName => {
          ...
          const selectorFilters = isNodeAdded ? this._addedFilters : isNodeRemoved ? this._removedFilters : this._changedFilters;
          if (!selectorFilters.length) return;
          const selectors = selectorFilters.map(filter => filter(target)).filter(Boolean);
          if (!selectors.length) return;
          const changedAttributes = this._watch.attributes ? Object.keys(previousAttributes).reduce((attributes, attrName) => {
  ...
  var documentModifications$1 = __AMD("core/js/DOMElementModifications", documentModifications);

  var TooltipView = __AMD("core/js/views/TooltipView", class TooltipView extends Backbone.View {
    className() {
    ...
    initialize() {
      ...
      this._tooltipData = {};
      this._tooltips = [];
      this.listenToOnce(Adapt$1, 'adapt:preInitialize', this.onAdaptPreInitialize);
      this.listenTo(documentModifications$1, 'added:[data-tooltip-id]', this.onAdded);
      this.render();
    }
    onAdaptPreInitialize() {
      ...
      $(document).on('focus', '[data-tooltip-id]', (...args) => {
        if (a11y$1.isForcedFocus) return;
    ...
    onMouseOver(event) {
      ...
      if (!a11y$1.isFocusable($mouseoverEl)) return;
    ...
    register(tooltipData) {
      if (!tooltipData._id) return logging$1.warn('Tooltip cannot be registered with no id');
  ...
  var tooltips$1 = __AMD("core/js/tooltips", tooltips);

  const DrawerCollection = new Backbone__default["default"].Collection(null, {
    comparator: 'drawerOrder'
  });
  class Drawer extends Backbone__default["default"].Controller {
    initialize() {
      this.listenTo(Adapt$1, {
        'adapt:start': this.onAdaptStart,
    ...
    onAdaptStart() {
      this._drawerView = new DrawerView$1({
    ...
    onLanguageChanged() {
      tooltips$1.register({
        _id: 'drawer',
        ...(Adapt$1.course.get('_globals')?._extensions?._drawer?._navTooltip || {})
      });
      this.remove();
    }
    toggle() {
  ...
  var drawer = __AMD("core/js/drawer", new Drawer());
  ...
  function toggleModelClass(model, className, hasClass) {
    const currentClassNames = (model.get('_classes') || '').split(' ').filter(Boolean);
    const classesObject = currentClassNames.reduce((classesObject, className) => ({
      ...classesObject,
      [className]: true
    }), {});
    const shouldToggleExisting = hasClass === null || hasClass === undefined;
    className.split(' ').filter(Boolean).forEach(className => {
      classesObject[className] = shouldToggleExisting ? !classesObject[className] : Boolean(hasClass);
    });
    const outputClasses = Object.entries(classesObject).filter(([, hasClass]) => hasClass).map(([className]) => className).join(' ');
  ...
  var AdaptModel = __AMD("core/js/models/adaptModel", class AdaptModel extends LockingModel {
    ...
    findRelativeModel(relativeString, options = {}) {
      ...
      const nextDescriptor = relativeDescriptorObjects.slice(1).reduce((output, {
        type,
        offset,
        inset
      }) => {
    ...
    getParent() {
      ...
      if (!parentId) return;
      const parent = data$1.findById(parentId);
      if (!parent) {
        logging$1.warn('adaptModel.getParent(): parent is empty');
        return;
      }
    ...
    checkLocking() {
      ...
      switch (lockType) {
        ...
        case 'custom':
          ...
          break;
        default:
          logging$1.warn(`AdaptModel.checkLocking: unknown _lockType '${lockType}' found on ${this.get('_id')}`);
      }
    }
    ...
    shouldLock(child) {
      ...
      return lockedBy.some(id => {
        try {
          ...
          logging$1.warn(`AdaptModel.shouldLock: unknown _lockedBy ID '${id}' found on ${child.get('_id')}`);
          return false;
        }
      });
    }
    ...
    deepClone(modifier = null) {
      ...
      clonedModel._lockedAttributes = {
        ...this._lockedAttributes
      };
      clonedModel._lockedAttributesValues = {
        ...this._lockedAttributesValues
      };
      if (modifier) {
  ...
  class AdaptView extends Backbone.View {
    ...
    setupOnScreenHandler() {
      ...
      this.$el.on('onscreen.adaptView', (e, m) => {
        ...
        Adapt$1.trigger(`${type}View:animationStart view:animationStart`, this);
      });
    }
    async addChildren() {
    ...
    addChildView(childView) {
      ...
      Adapt$1.trigger(`${type}View:childAdded view:childAdded`, this, childView);
      return childView;
    }
    ...
    _getAddChildEvent(model) {
      ...
      event.type = 'addChild';
      Adapt$1.trigger(`${type}View:addChild view:addChild`, event);
      event.close();
      return event;
    }
  ...
  class Router extends Backbone.Router {
    ...
    initialize({
      ...
      this.listenToOnce(Adapt$1, 'configModel:dataLoaded', this.onConfigLoaded);
    }
    get rootModel() {
    ...
    setDocumentTitle() {
      const currentModel = location$1._currentModel;
    ...
    async handleIdPreview(id) {
      ...
      if (!isContentObject) {
        ...
        let parentModel = Adapt$1.course;
        let _parentId = parentModel.get('_id');
        const built = buildTypes.map((_type, index) => {
          const ModelClass = components$1.getModelClass({
            _type
          });
        ...
        model.deepClone((clone, orig) => {
          ...
          clone.on('change', function () {
            const state = this.getAttemptObject ? this.getAttemptObject() : this.getTrackableState();
            delete state._id;
            delete state._isAvailable;
            delete state._isLocked;
            if (this.getAttemptObject) orig.setAttemptObject(state);else orig.set(state);
          });
    ...
    async removePreviews() {
      const previews = data$1.filter(model => model.get('_isPreview'));
      previews.forEach(model => data$1.remove(model));
    }
    async updateLocation(currentLocation, type, id, currentModel) {
      if (location$1._currentId === id && id === null) return;
    ...
    setGlobalClasses() {
      ...
      const classes = location$1._currentId ? `location-${location$1._contentType} location-id-${location$1._currentId}` : `location-${location$1._currentLocation}`;
    ...
    navigateToCurrentRoute(force) {
      ...
      const currentId = location$1._currentModel.get('_id');
      const isRoot = location$1._currentModel === this.rootModel;
    ...
    navigateToPreviousRoute(force) {
      ...
      const currentModel = location$1._currentModel;
      const previousModel = location$1._previousModel;
      if (!currentModel) {
        return this.navigateBack();
      }
      if (location$1._currentModel?.isTypeGroup('menu')) {
    ...
    navigateToParent(force) {
      ...
      const parentId = location$1._currentModel.get('_parentId');
    ...
    async navigateToElement(selector, settings = {}) {
      ...
      if (isSelectorAnId) {
        ...
        const isNotInCurrentContentObject = contentObjectId !== location$1._currentId;
        if (currentModel && (!currentModel.get('_isRendered') || !currentModel.get('_isReady') || isNotInCurrentContentObject)) {
          ...
          if (isNotInCurrentContentObject) {
            ...
            if (currentModel.isTypeGroup('contentobject')) {
              a11y$1.focusFirst(document.body);
              return;
            }
          }
      ...
      const newLocation = location$1._contentType ? location$1._contentType : location$1._currentLocation;
      ...
      await new Promise(resolve => {
        _.delay(() => {
          ...
          Adapt$1.trigger(`${newLocation}:scrolledTo`, selector);
          resolve();
    ...
    onConfigLoaded() {
      this.listenTo(Adapt$1.config, 'change:_activeLanguage', this.onLanguageChange);
    }
    onLanguageChange() {
  ...
  var NotifyPushView = __AMD("core/js/views/notifyPushView", class NotifyPushView extends Backbone.View {
    ...
    className() {
      const classes = ['notify-push', this.model.get('_classes'), this.model.get('_type') === 'a11y-push' && 'aria-label'].filter(Boolean).join(' ');
      return classes;
    }
    attributes() {
      return {
  ...
  var NotifyPopupView = __AMD("core/js/views/notifyPopupView", class NotifyPopupView extends Backbone__default["default"].View {
    ...
    initialize({
      ...
      dialog.addEventListener('mousedown', this.onShadowClicked, {
        capture: true
      });
    }
    setupEventListeners() {
  ...
  var NotifyView = __AMD("core/js/views/notifyView", class NotifyView extends Backbone.View {
    ...
    onDataReady() {
      const notifyDuration = Adapt$1.config.get('_notify')?._duration ?? 400;
  ...
  class StartController extends Backbone.Controller {
    ...
    getStartHash(alwaysForce) {
      ...
      if (shouldNavigateToStartId && startId !== Adapt$1.course.get('_id')) return `#/id/${startId}`;
  ...
  const COMPLETION_STATE$2 = ENUM([['NOT ATTEMPTED', 'NOTATTEMPTED'], 'INCOMPLETE', 'COMPLETED', 'PASSED', 'FAILED']);
  var COMPLETION_STATE$3 = __AMD("core/js/enums/completionStateEnum", COMPLETION_STATE$2);
  ...
  class Tracking extends Backbone.Controller {
    ...
    submitScore() {
      ...
      Adapt$1.course.set('_isAssessmentPassed', isPassed);
      if (!this._config._shouldSubmitScore) return;
      offlineStorage$1.set('score', this._assessmentState.score, this._assessmentState.minScore, this._assessmentState.maxScore, this._assessmentState.isPercentageBased);
    }
    checkCompletion() {
      ...
      if (!this._config._submitOnEveryAssessmentAttempt && completionData.status === COMPLETION_STATE$3.FAILED && canRetry) return;
      Adapt$1.trigger('tracking:complete', completionData);
      logging$1.debug('tracking:complete', completionData);
    }
    getCompletionData() {
      const completionData = {
        status: COMPLETION_STATE$3.INCOMPLETE,
        assessment: null
      };
      if (this._config._requireContentCompleted && !Adapt$1.course.get('_isComplete')) return completionData;
      if (this._config._requireAssessmentCompleted) {
        ...
        completionData.status = isPassed ? COMPLETION_STATE$3.PASSED : COMPLETION_STATE$3.FAILED;
        completionData.assessment = this._assessmentState;
        return completionData;
      }
      completionData.status = COMPLETION_STATE$3.COMPLETED;
      return completionData;
    }
    loadConfig() {
      Object.assign(this._config, Adapt$1.config.get('_completionCriteria'));
    ...
    onAssessmentComplete(assessmentState) {
      this.stopListening(Adapt$1, 'scoring:complete', this.onScoringComplete);
  ...
  Adapt$1.on({
    'drawer:remove'() {
      logging$1.deprecated('Use drawer.remove, Adapt.trigger(\'drawer:remove\') will be removed in the future');
      drawer.remove();
    },
    'drawer:closeDrawer'($toElement) {
      logging$1.deprecated('Use drawer.close, Adapt.trigger(\'drawer:closeDrawer\') will be removed in the future');
      drawer.close($toElement);
    },
    'drawer:triggerCustomView'() {
      logging$1.deprecated('Use drawer.openCustomView(), Adapt.trigger(\'drawer:triggerCustomView\') will be removed in the future');
  ...
  Object.defineProperties(drawer, {
    triggerCustomView: {
      get() {
        a11y$1.log.deprecated('drawer.triggerCustomView has moved to drawer.openCustomView');
  ...
  Object.defineProperties(Adapt$1, {
    ...
    offlineStorage: {
      get() {
        logging$1.deprecated('offlineStorage, please use core/js/offlineStorage instead');
        return offlineStorage$1;
      }
    },
    navigateToElement: {
      get() {
    ...
    wait: {
      get() {
        logging$1.deprecated('Adapt.wait, please use core/js/wait instead');
        return wait$1;
      }
    }
  });
  ...
  var NavigationButtonView = __AMD("core/js/views/NavigationButtonView", class NavigationButtonView extends Backbone.View {
    tagName() {
      return 'button';
    }
    events() {
    ...
    className() {
      if (this.isInjectedButton) {
        return [this.model.get('_showLabel') === false && 'hide-label'].filter(Boolean).join(' ');
      }
      return ['btn-icon nav__btn', this.model.get('_classes'), this.model.get('_showLabel') === false && 'hide-label'].filter(Boolean).join(' ');
    }
    attributes() {
  ...
  var NavigationButtonModel = __AMD("core/js/models/NavigationButtonModel", class NavigationButtonModel extends LockingModel {
    defaults() {
      return {
        _id: '',
        _classes: '',
  ...
  class NavigationView$1 extends Backbone.View {
    className() {
      return ['nav', this.model?.get('_showLabel') === true ? 'show-label' : 'hide-label', `show-label-${this.model?.get('_showLabelAtWidth') || 'medium'}`, `has-label-${this.model?.get('_labelPosition') || 'auto'}`].filter(Boolean).join(' ');
    }
    attributes() {
      return {
        role: 'navigation'
    ...
    initialize() {
      ___default["default"].bindAll(this, 'sortNavigationButtons');
      this._classSet = new Set(___default["default"].result(this, 'className').trim().split(/\s+/));
    }
    start(model) {
      tooltips$1.register({
        _id: 'back',
        ...(Adapt$1.course.get('_globals')?._extensions?._navigation?._backNavTooltip || {})
      });
      this.model = model;
      this.listenTo(model, 'change', this.update);
      this.listenToOnce(Adapt$1, 'courseModel:dataLoading', this.remove);
      this.listenTo(Adapt$1, {
    ...
    onDeviceResize() {
      ...
      const $html = $('html');
      const isBottomOnTouchDevices = device$1.touch && _isBottomOnTouchDevices;
      if (isBottomOnTouchDevices) _navigationAlignment = 'bottom';
      $html.removeClass('is-nav-top is-nav-bottom').addClass('is-nav-' + _navigationAlignment);
    }
    preRender() {
    ...
    render() {
      const template = Handlebars.templates[this.constructor.template];
      this.$el.html(template({
        _config: Adapt$1.config.toJSON(),
        _globals: Adapt$1.course.get('_globals'),
        _accessibility: Adapt$1.config.get('_accessibility')
      })).insertBefore('#app');
      this.sortNavigationButtons();
      ___default["default"].defer(() => {
        Adapt$1.trigger('navigationView:postRender navigation:ready', this);
      });
      return this;
    }
    updateViewProperties() {
      const classesToAdd = ___default["default"].result(this, 'className').trim().split(/\s+/);
      classesToAdd.forEach(i => this._classSet.add(i));
      const classesToRemove = [...this._classSet].filter(i => !classesToAdd.includes(i));
      classesToRemove.forEach(i => this._classSet.delete(i));
      this._setAttributes({
        ...___default["default"].result(this, 'attributes'),
        id: ___default["default"].result(this, 'id')
      });
      this.$el.removeClass(classesToRemove).addClass(classesToAdd);
    }
    ...
    sortNavigationButtons(changed) {
      if (Array.isArray(changed)) {
        ...
        const shouldIgnore = changes.every(([key]) => ['attributes.data-a11y-force-focus', 'attributes.tabindex', 'attributes.aria-hidden'].includes(key));
      ...
      items.forEach(el => {
        ...
        const foundId = ($el.attr('name') ?? Object.entries(identifiers).find(([classes]) => $el.is(classes))?.[1]) || $el.attr('class');
        const attributes = {
          _id: foundId,
  ...
  var NavigationModel = __AMD("core/js/models/NavigationModel", class NavigationModel extends LockingModel {
    defaults() {
      return {
        _navigationAlignment: 'top',
        _isBottomOnTouchDevices: false,
  ...
  class NavigationController extends Backbone.Controller {
    initialize() {
      this.navigation = new NavigationView$2();
  ...
  var navigation = __AMD("core/js/navigation", Adapt$1.navigation = new NavigationController().navigation);
  ...
  var CardsModel = __AMD("components/adapt-cards/js/CardsModel", class CardsModel extends ItemsComponentModel {
    defaults() {
  ...
  class CardsView extends ComponentView$1 {
    className() {
  ...
  var CardsView$1 = __AMD("components/adapt-cards/js/CardsView", CardsView);

  __AMD("components/adapt-cards/js/adapt-cards", components$1.register('cards', {
    model: CardsModel,
    view: CardsView$1
  }));

  var AccordionModel = __AMD("components/adapt-contrib-accordion/js/AccordionModel", class AccordionModel extends ItemsComponentModel {
    defaults() {
      return ItemsComponentModel.resultExtend('defaults', {
  ...
  class AccordionView extends ComponentView$1 {
    preRender() {
      this.onClick = this.onClick.bind(this);
  ...
  var AssessmentResultsModel = __AMD("components/adapt-contrib-assessmentResults/js/assessmentResultsModel", class AssessmentResultsModel extends ComponentModel$1 {
    init(...args) {
  ...
  class AssessmentResultsView extends ComponentView$1 {
    events() {
      return {
    ...
    preRender() {
      ...
      this.listenTo(Adapt$1.parentView, 'preRemove', () => {
    ...
    onRetryClicked() {
      ...
      Adapt$1.assessment.get(state.id).reset(null, wasReset => {
        ...
        router$1.navigateToElement(`.${state.articleId}`);
      });
    }
  ...
  class BlankView extends ComponentView$1 {
    postRender() {
      this.setReadyStatus();
      this.setupInviewCompletion();
    }
  ...
  var BlankView$1 = __AMD("components/adapt-contrib-blank/js/blankView", BlankView);
  ...
  var ButtonsView = __AMD("core/js/views/buttonsView", class ButtonsView extends Backbone.View {
    ...
    onButtonStateChanged(model, changedAttribute) {
      ...
      if (!buttonState) {
        return logging$1.error(`No button state found for '${changedAttribute}'`);
      }
    ...
    showMarking() {
      ...
      const correctnessAriaLabel = isCorrect ? ariaLabels.answeredCorrectly : isPartlyCorrect ? ariaLabels.answeredPartlyCorrect ?? ariaLabels.answeredIncorrectly : ariaLabels.answeredIncorrectly;
      if (!hasSpanAriaLabel) {
  ...
  class QuestionModel extends ComponentModel$1 {
    ...
    trackable() {
      return ComponentModel$1.resultExtend('trackable', ['_isSubmitted', '_score', '_isCorrect', '_attemptsLeft', '_isPartlyCorrect']);
    }
    trackableType() {
      return ComponentModel$1.resultExtend('trackableType', [Boolean, Number, Boolean, Number, Boolean]);
    }
    lockedAttributes() {
      return ComponentModel$1.resultExtend('lockedAttributes', {
        _canSubmit: true
      });
    }
    getTypeGroup() {
    ...
    getFeedback(feedback = this.get('_feedback')) {
      ...
      const isCorrect = this.get('_isCorrect');
      const correctness = isCorrect ? 'correct' : this.isPartlyCorrect() ? 'partlyCorrect' : 'incorrect';
      const isLegacyConfig = typeof feedback.correct === 'string' || typeof feedback._partlyCorrect === 'object' || typeof feedback._incorrect === 'object';
      const getLegacyConfigObject = () => {
        const subPart = isFinal ? 'final' : 'notFinal';
        return {
          body: (isCorrect ? feedback.correct : feedback[`_${correctness}`]?.[subPart] || feedback[`_${correctness}`]?.final || feedback._incorrect?.final) || ''
        };
      };
      const getConfigObject = () => {
        const subPart = isFinal ? 'Final' : 'NotFinal';
        return (isCorrect ? feedback._correct : feedback[`_${correctness}${subPart}`] || feedback[`_${correctness}Final`] || feedback._incorrectFinal) || {};
      };
      const altFeedbackTitle = Adapt$1.course.get('_globals')._accessibility.altFeedbackTitle;
      ...
      const feedbackConfig = {
        ...
        ...(isLegacyConfig ? getLegacyConfigObject() : getConfigObject())
      };
    ...
    getButtonState() {
      ...
      if (this.get('_attemptsLeft') === 0) {
        const canShowModelAnswer = this.get('_canShowModelAnswer');
        const canShowCorrectness = this.get('_canShowCorrectness');
        return canShowModelAnswer && !canShowCorrectness ? BUTTON_STATE$1.SHOW_CORRECT_ANSWER : BUTTON_STATE$1.INCORRECT;
      }
  ...
  class QuestionView extends ComponentView$1 {
    ...
    onSubmitClicked() {
      ...
      this.onSubmitted();
      Adapt$1.trigger('questionView:submitted', this);
    }
    ...
    onResetClicked() {
      ...
      this._runModelCompatibleFunction('updateButtons');
      if (!this.model.get('_isReady')) return;
      const currentModel = data$1.findById(location$1._currentId);
  ...
  class ViewOnlyQuestionViewCompatibilityLayer extends QuestionView {
    ...
    getResponse() {
      logging$1.deprecated('QuestionView.getResponse, please use QuestionModel.getResponse');
  ...
  class McqView extends QuestionView$1 {
    initialize(...args) {
      ...
      this.onKeyPress = (...args) => {
        logging$1.deprecated('McqView onKeyPress is deprecated, please change to onKeyDown');
        this.onKeyDown(...args);
      };
  ...
  class GmcqView extends McqView$1 {
    onQuestionRendered() {
  ...
  var ItemsQuestionModel = __AMD("core/js/models/itemsQuestionModel", class ItemsQuestionModel extends BlendedItemsComponentQuestionModel {
    init() {
      super.init();
  ...
  var McqModel = __AMD("components/adapt-contrib-mcq/js/McqModel", class McqModel extends ItemsQuestionModel {});

  var GmcqModel = __AMD("components/adapt-contrib-gmcq/js/GmcqModel", class GmcqModel extends McqModel {});
  ...
  class HotgraphicPopupView extends Backbone.View {
    ...
    setupEventListeners() {
      this.listenToOnce(Adapt$1, 'notify:opened', this.onNotifyOpened);
    ...
    setupBackNextLabels(index = this.model.getActiveItem().get('_index')) {
      ...
      const globals = Adapt$1.course.get('_globals');
    ...
    updatePageCount() {
      ...
      const totalItems = this.model.getChildren().length;
      const popupCount = compile(template, {
        itemNumber,
        totalItems
      });
    ...
    render() {
      ReactDOM__default["default"].render(React__default["default"].createElement(templates.hotgraphicPopup, this.model.toJSON()), this.el);
    }
  }
  var HotgraphicPopupView$1 = __AMD("components/adapt-contrib-hotgraphic/js/hotgraphicPopupView", HotgraphicPopupView);

  class HotGraphicView extends ComponentView$1 {
    initialize(...args) {
      super.initialize(...args);
    ...
    reRender() {
      if (device$1.isScreenSizeMin('medium') || this.model.get('_isNarrativeOnMobile') === false) return;
      _.defer(() => {
        this.replaceWithNarrative();
      });
    }
    replaceWithNarrative() {
      const NarrativeView = components$1.getViewClass('narrative');
      if (!NarrativeView) return;
      const model = this.model.prepareNarrativeModel();
      const newNarrative = new NarrativeView({
        model
      });
      const parentView = data$1.findViewByModelId(model.get('_parentId'));
  ...
  var HotgraphicModel = __AMD("components/adapt-contrib-hotgraphic/js/hotgraphicModel", class HotgraphicModel extends ItemsComponentModel {
    defaults() {
      return ItemsComponentModel.resultExtend('defaults', {
    ...
    setUpItems() {
      ...
      this.getChildren().forEach((child, index) => {
        ...
        tooltip._classes = [tooltip._classes, 'hotgraphic__pin-tooltip'].filter(Boolean).join(' ');
        const tooltipConfig = {
        ...
        const tooltipModel = tooltips$1.register(tooltipConfig);
        child.on('change', () => {
          tooltipModel.set({
            ...child.toJSON(),
            ...tooltip
  ...
  var MatchingModel = __AMD("components/adapt-contrib-matching/js/MatchingModel", class MatchingModel extends ItemsQuestionModel {
    toJSON() {
      const json = super.toJSON();
    ...
    setUpItems() {
      ...
      items.forEach(item => {
        const itemOptions = item._options || [];
        item._correctAnswers = itemOptions.filter(option => option._isCorrect).map(option => option.text || '').map(item => item.trim()).filter(Boolean);
      });
      this.set({
        _items: items,
  ...
  window.mejs.MediaElementPlayer.prototype.buildfullscreen = function (player, controls, layers, media) {
    ...
    const fullscreenBtn = $('<div class="mejs-button mejs-fullscreen-button">' + '<button type="button" aria-controls="' + t.id + '" title="' + t.options.fullscreenText + '" aria-label="' + t.options.fullscreenText + '"></button>' + '</div>').appendTo(controls).on('click', function () {
  ...
  if (mediaFeatures.hasMozNativeFullScreen) {
    Object.assign(mediaFeatures, {
      fullScreenEventName: document.exitFullscreen ? 'fullscreenchange' : 'mozfullscreenchange',
      requestFullScreen: el => {
        document.exitFullscreen ? el.requestFullscreen() : el.mozRequestFullScreen();
      },
      isFullScreen: () => {
        return document.exitFullscreen ? Boolean(document.fullscreenElement) : document.mozFullScreen;
      },
      cancelFullScreen: el => {
  ...
  Adapt$1.on('app:dataReady', () => {
    ...
    const globals = Adapt$1.course.get('_globals')?._components?._media;
  ...
  class MediaView extends ComponentView$1 {
    events() {
      return {
    ...
    setupPlayer() {
      ...
      this.addThirdPartyFixes(modelOptions, () => {
        ...
        if (!_media.mp3 && !_media.mp4 && !_media.ogv && !_media.webm && !_media.source) {
          logging$1.warn('ERROR! No media is selected in components.json for component ' + this.model.get('_id'));
          this.setReadyStatus();
          return;
        }
    ...
    addThirdPartyFixes(modelOptions, callback) {
      ...
      switch (media.type) {
        case 'video/vimeo':
          ...
          $.getScript('assets/froogaloop.js').done(() => {
    ...
    listenForCaptionsChange() {
      ...
      const selector = this.model.get('_playerOptions').toggleCaptionsButtonWhenOnlyOne ? '.mejs-captions-button button' : '.mejs-captions-selector';
      this.$(selector).on('click.mediaCaptionsChange', _.debounce(() => {
        ...
        Adapt$1.trigger('media:captionsChange', this, srclang);
      }, 250));
    ...
    onMediaVolumeChange(event) {
      Adapt$1.trigger('media:volumeChange', this.model, this.mediaElement.muted, this.mediaElement.volume);
    }
    ...
    onSkipToTranscript() {
      _.delay(() => {
        a11y$1.focus(this.$('.media__transcript-btn'));
      }, 250);
    }
    ...
    transcriptTriggers(state) {
      ...
      if (!shouldComplete) {
        return Adapt$1.trigger('media:transcript', state, this);
      }
      this.setCompletionStatus();
      Adapt$1.trigger('media:transcript', 'complete', this);
    }
  ...
  var NarrativeModel = __AMD("components/adapt-contrib-narrative/js/NarrativeModel", class NarrativeModel extends ItemsComponentModel {
    restoreUserAnswers() {
  ...
  class NarrativeView extends ComponentView$1 {
    events() {
    ...
    async setFocus(itemIndex) {
      const $animatedElement = this.isLargeMode() || this.model.get('_isMobileTextBelowImage') ? this.$('.narrative__slider') : this.$('.narrative__strapline-header-inner');
    ...
    focusOnNarrativeElement(itemIndex) {
      ...
      const $elementToFocus = this.isLargeMode() || this.model.get('_isMobileTextBelowImage') ? this.$(`.narrative__content-item${dataIndexAttr}`) : this.$(`.narrative__strapline-btn${dataIndexAttr}`);
    ...
    calculateMode() {
      const mode = device$1.isScreenSizeMin('medium') ? MODE.LARGE : MODE.SMALL;
    ...
    isTextBelowImage() {
      const isTextBelowImage = device$1.isScreenSizeMin('medium') ? this.model.get('_isTextBelowImage') : this.model.get('_isMobileTextBelowImage');
    ...
    replaceWithHotgraphic() {
      const HotgraphicView = components$1.getViewClass('hotgraphic');
      if (!HotgraphicView) return;
      const model = this.prepareHotgraphicModel();
      const newHotgraphic = new HotgraphicView({
        model
      });
      const parentView = data$1.findViewByModelId(model.get('_parentId'));
      const $container = parentView.$el.find('.component__container');
    ...
    shouldUseInviewCompletion() {
      ...
      if (!isMobileAndStacked && setCompletionOn !== 'inview') {
        return false;
      }
      return true;
    }
  ...
  __AMD("components/adapt-contrib-narrative/js/adapt-contrib-narrative", components$1.register('narrative', {
    model: NarrativeModel,
    view: NarrativeView$1
  }));

  class SliderView extends QuestionView$1 {
    initialize(...args) {
  ...
  var SliderModel = __AMD("components/adapt-contrib-slider/js/SliderModel", class SliderModel extends QuestionModel$1 {
    defaults() {
      return QuestionModel$1.resultExtend('defaults', {
        _scaleStart: 1,
        _scaleEnd: 10,
        _scaleStep: 1,
        _showScale: true,
    ...
    init() {
      ...
      if (this.get('_scaleStep') <= 0) {
        logging$1.warn(`\`_scaleStep\` must be a positive number, restoring default of 1 for ${this.get('_id')}`);
    ...
    isCorrect() {
      const numberOfCorrectAnswers = this.get('_items').filter(({
        selected,
        correct
      }) => selected && correct).length;
    ...
    getCorrectAnswerAsText() {
      const globals = Adapt$1.course.get('_globals')._components._slider;
    ...
    getUserAnswerAsText() {
      const answerTemplate = Adapt$1.course.get('_globals')._components._slider.ariaUserAnswer;
  ...
  class TextInputView extends QuestionView$1 {
    events() {
      return {
  ...
  class TextInputModel extends QuestionModel$1 {
    init() {
      super.init();
    ...
    setupCorrectAnswers() {
      ...
      this.get('_items').forEach(item => {
        ...
        const answers = hasItemAnswers ? itemAnswers.flatMap(items => items || []) : genericAnswers.flatMap(items => items || []);
    ...
    storeUserAnswer() {
      ...
      items.forEach(({
        _index,
        _answerIndex
      }) => userAnswer[_index] = _answerIndex);
      this.set('_userAnswer', userAnswer);
    }
    isCorrect() {
  ...
  var CountersModel = __AMD("components/adapt-counters/js/CountersModel", class CountersModel extends ItemsComponentModel {
    defaults() {
      return ItemsComponentModel.resultExtend('defaults', {
        _animateItems: false,
        _percentInviewVertical: 70,
  ...
  class CountersView extends ComponentView$1 {
    postRender() {
  ...
  var CountersView$1 = __AMD("components/adapt-counters/js/CountersView", CountersView);

  __AMD("components/adapt-counters/js/adapt-counters", components$1.register('counters', {
    model: CountersModel,
    view: CountersView$1
  }));
  ...
  var Expose = ComponentView$1.extend({
    events: {
  ...
  Adapt$1.register('expose', Expose);
  __AMD("components/adapt-expose/js/adapt-expose", Expose);

  var Flipper = ComponentView$1.extend({
    events: {
  ...
  Adapt$1.register('flipper', Flipper);
  __AMD("components/adapt-flipper/js/adapt-flipper", Flipper);

  var FlowItemsModel = __AMD("components/adapt-flowItems/js/FlowItemsModel", class FlowItemsModel extends ItemsComponentModel {
    setActive(index) {
      const item = this.getItem(index);
      this.resetActiveItems();
      if (index === 0) {
  ...
  class FlowItemsView extends ComponentView$1 {
    className() {
      let classes = super.className();
  ...
  var FlowItemsView$1 = __AMD("components/adapt-flowItems/js/FlowItemsView", FlowItemsView);

  __AMD("components/adapt-flowItems/js/adapt-flowItems", Adapt$1.register('flowItems', {
    model: FlowItemsModel,
    view: FlowItemsView$1
  }));

  var ListModel = __AMD("components/adapt-list/js/ListModel", class ListModel extends ItemsComponentModel {
    defaults() {
      return ItemsComponentModel.resultExtend('defaults', {
        _animateList: false,
        _columns: 0,
  ...
  class ListView extends ComponentView$1 {
    className() {
      let classes = super.className();
  ...
  var ListView$1 = __AMD("components/adapt-list/js/ListView", ListView);

  __AMD("components/adapt-list/js/adapt-list", components$1.register('list', {
    model: ListModel,
    view: ListView$1
  }));

  class MemoryView extends ComponentView$1 {
    preRender() {
  ...
  var MemoryView$1 = __AMD("components/adapt-memory/js/MemoryView", MemoryView);

  var MemoryModel = __AMD("components/adapt-memory/js/MemoryModel", class MemoryModel extends ItemsComponentModel {
    setUpItems() {
      const items = this.get('_items') || [];
      items.forEach((item, index) => item._index = index + 1);
      this.setChildren(new Backbone.Collection(_.shuffle(items), {
        model: ItemModel
      }));
    }
    restoreUserAnswers() {
      ...
      this.setChildren(new Backbone.Collection(newItems, {
        model: ItemModel
      }));
    }
    storeUserAnswer() {
  ...
  __AMD("components/adapt-memory/js/adapt-memory", components$1.register('memory', {
    model: MemoryModel,
    view: MemoryView$1
  }));

  function onDataReady() {
    var isEnabledOnCourse = isNotifyEnabled(Adapt$1.course);
    if (isEnabledOnCourse) {
      Adapt$1.listenTo(Adapt$1.course, "change:_isComplete", onCourseComplete);
    }
    _.each(Adapt$1.contentObjects.models, function (model) {
      if (isNotifyEnabled(model)) {
        Adapt$1.listenTo(model, "change:_isComplete", onContentObjectComplete);
      }
    });
  }
  ...
  function onContentObjectComplete(pageModel) {
    if (Adapt$1.course.get("_isComplete")) {
      return;
    }
    var notifyConfig = pageModel.get("_notifyOnComplete");
    Adapt$1.trigger("notify:push", {
  ...
  Adapt$1.once("app:dataLoaded", onDataReady);

  var SliderItemsModel = __AMD("components/adapt-sliderItems/js/SliderItemsModel", class SliderItemsModel extends ItemsComponentModel {
    setActive(index) {
      this.resetActiveItems();
      const item = this.getItem(index);
      item.set({
  ...
  class SliderItemsView extends ComponentView$1 {
    initialize(...args) {
  ...
  var SliderItemsView$1 = __AMD("components/adapt-sliderItems/js/SliderItemsView", SliderItemsView);

  __AMD("components/adapt-sliderItems/js/adapt-sliderItems", Adapt$1.register('sliderItems', {
    model: SliderItemsModel,
    view: SliderItemsView$1
  }));

  class TabsView extends ComponentView$1 {
    className() {
      return [super.className(), `is-${this.layout}-layout`].join(' ');
    }
    get layout() {
      const layout = device$1.isScreenSizeMin('large') ? this.model.get('_tabLayout') : 'vertical';
      return layout;
    }
    preRender() {
      this.reset();
      ...
      this.listenTo(Adapt$1, 'device:resize', this.setItemWidth);
      this.listenTo(this.model.getChildren(), 'change:_isActive', this.onItemsActiveChange);
    }
    reset() {
      this.model.resetActiveItems();
    ...
    setItemWidth() {
      const itemWidth = this.layout === 'horizontal' ? 100 / this.model.get('_items').length + '%' : '';
    ...
    onTabItemKeyUp(event) {
      ...
      a11y$1.focus($tabs[currentIndex]);
    }
  }
  TabsView.template = 'tabs.jsx';
  var TabsView$1 = __AMD("components/adapt-tabs/js/TabsView", TabsView);

  __AMD("components/adapt-tabs/js/adapt-tabs", components$1.register('tabs', {
    model: ItemsComponentModel.extend({}),
    view: TabsView$1
  }));

  var TalkModel = __AMD("components/adapt-talk/js/TalkModel", class TalkModel extends ComponentModel$1 {
    init() {
      this.setUpItems();
      super.init();
    }
  ...
  class TalkView extends ComponentView$1 {
    postRender() {
      this.$('.component__widget').imageready(() => {
        this.setReadyStatus();
      });
  ...
  var TalkView$1 = __AMD("components/adapt-talk/js/TalkView", TalkView);

  __AMD("components/adapt-talk/js/adapt-talk", components$1.register('talk', {
    model: TalkModel,
    view: TalkView$1
  }));

  class TypewriterView extends ComponentView$1 {
    postRender() {
      this.setReadyStatus();
      this.setupInviewCompletion('.component__widget');
    }
  ...
  var TypewriterView$1 = __AMD("components/adapt-typewriter/js/TypewriterView", TypewriterView);

  var TypewriterModel = __AMD("components/adapt-typewriter/js/TypewriterModel", class TypewriterModel extends ComponentModel$1 {});

  __AMD("components/adapt-typewriter/js/adapt-typewriter", components$1.register('typewriter', {
    model: TypewriterModel,
    view: TypewriterView$1
  }));

  var VerticalItemsModel = __AMD("components/adapt-verticalItems/js/VerticalItemsModel", class VerticalItemsModel extends ItemsComponentModel {});

  class VerticalItemsView extends ComponentView$1 {
    postRender() {
      this.setupItems();
      this.setReadyStatus();
      this.setupInviewCompletion('.component__widget');
  ...
  var VerticalItemsView$1 = __AMD("components/adapt-verticalItems/js/VerticalItemsView", VerticalItemsView);

  __AMD("components/adapt-verticalItems/js/adapt-verticalItems", Adapt$1.register('verticalItems', {
    model: VerticalItemsModel,
    view: VerticalItemsView$1
  }));
  ...
  var AboutUsItemView = Backbone.View.extend({
    ...
    onAboutUsItemClicked: function (event) {
      ...
      Adapt$1.trigger('aboutUs:descriptionOpen', this.model.cid);
    },
    ...
    showAboutUsItemDescription: function () {
      ...
      $aboutUsItemTopic.addClass('is-selected').siblings('.aboutus__item-body').slideDown(200, function () {
        Adapt$1.a11y.focusFirst(this.$el, {
          defer: true
        });
      }.this);
    ...
    hideAboutUsItemDescription: function () {
      this.$('.aboutus__item-body').stop(true, true).slideUp(200);
      this.model.set('_isDescriptionOpen', false);
      this.$('.js-aboutus-item-topic-click').removeClass('is-selected').attr('aria-expanded', false);
    },
  ...
  var AboutUsItemView$1 = __AMD("extensions/adapt-aboutUs/js/adapt-aboutUsItemView", AboutUsItemView);
  ...
  var AboutUsSocialLinksView = Backbone.View.extend({
    ...
    postRender: function () {
      this.listenTo(Adapt$1, 'drawer:openedItemView', this.remove);
      this.listenTo(Adapt$1, 'drawer:triggerCustomView', this.remove);
    }
  });
  var SocialLinksView = __AMD("extensions/adapt-aboutUs/js/adapt-aboutUsSocialLinksView", AboutUsSocialLinksView);
  ...
  var AboutUsView = Backbone.View.extend({
    ...
    renderAboutUsItems: function () {
      ...
      _.each(this.collection.models, function (item, index) {
        var itemView = new AboutUsItemView$1({
          model: item
        });
        itemView.$el.appendTo($aboutUsItemContainer);
      }, this);
      new SocialLinksView({
        model: Adapt$1.course.get('_aboutUs')._socialLinks
  ...
  var AboutUsView$1 = __AMD("extensions/adapt-aboutUs/js/adapt-aboutUsView", AboutUsView);
  ...
  function setupAboutUs(aboutUsModel, aboutUsItems, socialLinks) {
    ...
    Adapt$1.on('aboutUs:showAboutUs', function () {
      Adapt$1.drawer.triggerCustomView(new AboutUsView$1(options).$el);
    });
  }
  function initAboutUs() {
    var courseAboutUs = Adapt$1.course.get('_aboutUs');
    ...
    Adapt$1.drawer.addItem(drawerObject, 'aboutUs:showAboutUs');
    setupAboutUs(courseAboutUs, courseAboutUs._aboutUsItems, courseAboutUs._socialLinks);
  }
  Adapt$1.on('app:dataReady', initAboutUs);
  ...
  class QuestionBank {
    ...
    getRandomQuestionBlocks() {
      ...
      if (count > this.unusedQuestionBlocks.length) {
        logging$1.warn(`Bank ID ${this._bankId}: Attempting to display ${count} question(s), but only ${this.unusedQuestionBlocks.length} available`);
  ...
  const AssessmentModel = {
      ...
      Object.defineProperty(this, '_originalChildModels', {
        get() {
          if (this._originalChildModelsStore) {
            this._originalChildModelsStore = this._originalChildModelsStore.filter(model => data$1.hasId(model.get('_id')));
    ...
    _onDataReady() {
      ...
      if (this.get('_attemptInProgress') && questions.every(q => q.get('_isSubmitted')) && this.get('_requireCompletionOf') !== Number.POSITIVE_INFINITY) {
        ...
        logging$1.warn(`Assessment ${this.get('_id')} _attemptInProgress corrected as all questions are submitted`);
      }
    },
    ...
    _setupBanks() {
      ...
      this.findDescendantModels('block').filter(block => block.get('_isAvailable') && block.findDescendantModels('question').length > 0).forEach(block => {
        ...
        if (isInvalidNumber) logging$1.warn(`Bank ID ${quizBankId} is not a valid number`);
        if (isOutOfBounds) logging$1.warn(`Bank ID ${quizBankId} exceeds the number of available splits (${bankSplits.length})`);
      });
    ...
    _checkReloadPage() {
      ...
      const currentLocation = location$1._currentId;
  ...
  var BookmarkingModel = __AMD("extensions/adapt-contrib-bookmarking/js/BookmarkingModel", class BookmarkingModel extends Backbone.Model {
    defaults() {
      return {
        label: '',
        ariaLabel: '',
  ...
  var BookmarkingView = __AMD("extensions/adapt-contrib-bookmarking/js/BookmarkingView", class BookmarkingView extends Backbone.View {
    get config() {
      return Adapt$1.course.get('_bookmarking');
    }
    className() {
      return classes(['bookmarking-resume']);
    }
    events() {
      return {
    ...
    onResumeClicked() {
      Adapt$1.trigger('bookmarking:resume', this.model.get('_location'));
    }
  });
  ...
  class Bookmarking extends Backbone.Controller {
    ...
    get globals() {
      return Adapt$1.course.get('_globals');
    }
    get config() {
      return Adapt$1.course.get('_bookmarking');
    }
    ...
    getLocationId(location = this.location) {
      const id = location === 'furthest' ? this.furthestIncompleteModel?.get?.('_id') : this.restoredLocationID;
      return id;
    }
    get furthestIncompleteModel() {
      const bookmarkLevel = this.config._level || 'component';
      const getIncompleteModels = Adapt$1.course.findDescendantModels(bookmarkLevel, {
        where: {
          _isComplete: false,
          _isAvailable: true,
          _isOptional: false
    ...
    setupEventListeners() {
      ...
      this.listenTo(documentModifications$1, {
    ...
    checkRestoreLocation() {
      ...
      if (!this.restoredLocationID || this.restoredLocationID === 'undefined' || !data$1.findById(this.restoredLocationID)) return;
    ...
    isAlreadyOnScreen(id) {
      ...
      const type = data$1.findById(id).getTypeGroup();
    ...
    showPrompt() {
      const buttons = this.config._buttons || {
        yes: 'Yes',
        no: 'No'
      };
      this.listenToOnce(Adapt$1, {
    ...
    resolveLocking(id) {
      ...
      const descendants = isContentObject ? Adapt$1.course.getAllDescendantModels(true).filter(descendant => descendant.isTypeGroup('contentobject')) : model.findAncestor('page').getAllDescendantModels(true);
    ...
    navigateTo(location = this.location) {
      ...
      _.defer(async () => {
        try {
          const isSinglePage = Adapt$1.contentObjects.models.length === 1;
          await router$1.navigateToElement(target, {
            trigger: true,
            replace: isSinglePage,
            duration: 400
          });
        } catch (err) {
          logging$1.warn(`Bookmarking cannot navigate to id: ${target}\n`, err);
        }
      });
    ...
    checkLocation() {
      const contentObjectBookmarkModel = Adapt$1.parentView?.model?.get('_bookmarking');
      ...
      const possibleViewIds = currentModel.findDescendantModels(this.bookmarkLevel).filter(desc => desc.get('_isTrackable') !== false).map(desc => desc.get('_id'));
  ...
  var NavigationView = __AMD("extensions/adapt-contrib-languagePicker/js/languagePickerNavigationView", class NavigationView extends Backbone.View {
    className() {
      return 'nav';
    }
    attributes() {
    ...
    render() {
      ...
      _.defer(() => Adapt$1.trigger('navigationView:postRender', this));
      return this;
    }
    setupHelpers() {
  ...
  var LanguagePickerView = __AMD("extensions/adapt-contrib-languagePicker/js/languagePickerView", class LanguagePickerView extends Backbone.View {
    get template() {
  ...
  var LanguagePickerDrawerView = __AMD("extensions/adapt-contrib-languagePicker/js/languagePickerDrawerView", class LanguagePickerDrawerView extends Backbone.View {
    get template() {
    ...
    onDrawerClosed() {
      _.delay(() => {
        ...
        notify$1.prompt(this.promptObject);
      }, 250);
    }
    onPopupOpened() {
    ...
    onDontChangeLanguage() {
      this.remove();
      _.delay(() => a11y$1.focusFirst(this.$finishFocus), 500);
    }
  ...
  var LanguagePickerNavigationButton = __AMD("extensions/adapt-contrib-languagePicker/js/languagePickerNavigationButton", class LanguagePickerNavigationButton extends NavigationButtonView {
    attributes() {
      return {
        ...super.attributes(),
    ...
    onClick(event) {
      drawer.openCustomView(new LanguagePickerDrawerView({
        model: this.drawerModel
      }).$el, false, this.model.get('_drawerPosition'));
    }
  });

  var LanguagePickerModel = __AMD("extensions/adapt-contrib-languagePicker/js/languagePickerModel", class LanguagePickerModel extends Backbone.Model {
    preinitialize() {
      this.trackedData = {
        components: [],
        blocks: []
    ...
    getLanguageDetails(language) {
      return this.get('_languages').find(({
        _language
      }) => _language === language);
    }
    setLanguage(language, {
      ...
      this.locationId = offlineStorage$1.get('location') || null;
      Adapt$1.config.set({
    ...
    checkResetOnLanguageChange() {
      ...
      const hasOfflineStorageClear = Boolean(offlineStorage$1.clear);
      if (!shouldReset || !hasOfflineStorageClear) return;
      offlineStorage$1.clear();
      this.locationId = null;
    }
    ...
    restoreLocation() {
      ...
      _.defer(() => router$1.navigateToElement('.' + this.locationId));
    }
    restoreState() {
    ...
    setTrackableState(stateObject) {
      const restoreModel = data$1.findById(stateObject._id);
      if (!restoreModel) {
  ...
  class LanguagePicker extends Backbone.Controller {
    ...
    onConfigLoaded() {
      const config = Adapt$1.config.get('_languagePicker');
      if (!config?._isEnabled) return;
    ...
    async onOfflineStorageReady() {
      await Adapt$1.wait.queue();
      const storedLanguage = offlineStorage$1.get('lang');
      if (storedLanguage) {
    ...
    setupNavigationView() {
      const globalsConfig = Adapt$1.course.get('_globals')?._extensions?._languagePicker;
      const {
        _navOrder = 0,
        _showLabel = true,
        navigationBarLabel = '',
  ...
  class Completion extends Backbone.Controller {
    initialize() {
      ___default["default"].bindAll(this, 'calculatePercentageComplete');
    ...
    calculateCompletion(contentObjectModel, setGlobal = false) {
      ...
      if (setGlobal) Object.assign(Adapt$1.completion, completion);
      return completion;
    }
  ...
  var PageLevelProgressView = __AMD("extensions/adapt-contrib-pageLevelProgress/js/PageLevelProgressView", class PageLevelProgressView extends Backbone.View {
    className() {
      const config = Adapt$1.course.get('_pageLevelProgress');
      return ['pagelevelprogress', config._showAtCourseLevel === true && 'is-course-level'].filter(Boolean).join(' ');
    }
    events() {
      return {
    ...
    render() {
      ...
      ReactDOM__default["default"].render(React__default["default"].createElement(Component, {
  ...
  class PageLevelProgressIndicatorView extends Backbone.View {
    ...
    addClasses() {
      this.$el.addClass(['pagelevelprogress__indicator-outer', 'is-' + this.type].join(' '));
    }
    setUpEventListeners() {
      if (this.parent) {
      ...
      this.listenTo(Adapt$1.course, 'bubble:change:_isComplete bubble:change:_isVisited', this.render);
    }
    ...
    render() {
      ...
      ReactDOM__default["default"].render(React__default["default"].createElement(Component, data), this.el);
    }
    refresh() {
      this.render();
    }
    checkCompletion() {
      ...
      this.$el.toggleClass('is-complete', isComplete).toggleClass('is-incomplete', !isComplete).toggleClass('is-correct', isCorrect).toggleClass('is-partially-correct', isPartlyCorrect).toggleClass('is-incorrect', isIncorrect);
    }
    checkAriaHidden() {
  ...
  var getPageLevelProgressItems = __AMD("extensions/adapt-contrib-pageLevelProgress/js/getPageLevelProgressItems", function getPageLevelProgressItemsJSON(parentModel) {
    const {
      _showAtCourseLevel
    ...
    const jsonOutput = _showAtCourseLevel ? addChildren(Adapt$1.course) : addChildren(parentModel)._children;
    return jsonOutput;
  });

  var PageLevelProgressNavigationView = __AMD("extensions/adapt-contrib-pageLevelProgress/js/PageLevelProgressNavigationView", class PageLevelProgressNavigationView extends NavigationButtonView {
    events() {
      return {
    ...
    initialize(options) {
      ...
      tooltips$1.register({
        ...
        ...(Adapt$1.course.get('_globals')?._extensions?._pageLevelProgress?._navTooltip || {})
      });
      this.tooltip = tooltips$1.getTooltip('pagelevelprogress');
    }
    setUpEventListeners() {
      this.listenTo(Adapt$1, {
        remove: this.remove,
      ...
      this.listenTo(data$1, 'change:_isLocked change:_isVisible change:_isComplete', this.refreshProgressBar);
    }
    addIndicator() {
      const {
      ...
      this.indicatorView = new PageLevelProgressIndicatorView$1({
        model: _useCourseProgressInNavigationButton ? Adapt$1.course : this.pageModel,
    ...
    _getPageCompletionPercentage() {
      ...
      return _useCourseProgressInNavigationButton ? completionCalculations.calculatePercentageComplete(Adapt$1.course) : completionCalculations.calculatePercentageComplete(this.pageModel, true);
    }
    deferredUpdate() {
    ...
    refreshProgressBar() {
      ...
      this.collection = getPageLevelProgressItems(this.pageModel);
    ...
    onProgressClicked(event) {
      ...
      Adapt$1.trigger('pageLevelProgress:toggleDrawer');
    }
    remove() {
      super.remove();
    }
  ...
  class PageLevelProgress extends Backbone.Controller {
    ...
    static get globalsConfig() {
      return Adapt$1.course.get('_globals')?._extensions?._pageLevelProgress;
    }
    ...
    setUpEventListeners() {
      ...
      this.listenTo(Adapt$1.course, 'bubble:change:_isComplete bubble:change:_isVisited', this.onCompletionChange);
    }
    ...
    renderNavigationView(pageModel) {
      const navigationConfig = Adapt$1.course.get('_navigation');
  ...
  var ResourcesView = __AMD("extensions/adapt-contrib-resources/js/ResourcesView", class ResourcesView extends Backbone.View {
    className() {
      return 'resources';
    }
    initialize() {
    ...
    render() {
      ...
      ReactDOM__default["default"].render(React__default["default"].createElement(templates.resources, data), this.el);
      _.defer(() => {
        Adapt$1.trigger('view:render', this);
        this.listenTo(Adapt$1, 'drawer:closed', this.remove);
      });
      return this;
    }
    onResourceItemClicked(e) {
      ...
      Adapt$1.trigger('resources:itemClicked', resourceItemData);
    }
  });
  ...
  class Resources extends Backbone.Controller {
    ...
    initResources() {
      const courseResources = Adapt$1.course.get('_resources');
    ...
    setupResources(resourcesData) {
      this.listenTo(Adapt$1, 'resources:showResources', () => {
        const model = new Backbone.Model(resourcesData);
        let resources = model.get('_resourcesItems');
        const contentObjectModel = Adapt$1.parentView?.model;
        const contentObjectConfig = contentObjectModel?.get('_resources');
        const isContentObjectNotCourseModel = contentObjectModel !== Adapt$1.course;
    ...
    setupTypes(model, resourcesData) {
      ...
      const typesWithItems = configuredTypes.filter(type => {
        return resourcesData._resourcesItems.some(_.matcher({
          _type: type
        }));
      });
    ...
    setupFilters(model, resources) {
      ...
      const hasMultipleTypes = !resources.every(_.matcher({
    ...
    setupCanDownload(model) {
      const canEnableDownloads = device$1.browser !== 'internet explorer' && device$1.OS !== 'ios';
  ...
  const COMPLETION_STATE = ENUM(['UNKNOWN', ['NOT ATTEMPTED', 'NOTATTEMPTED'], 'NOT_ATTEMPTED', 'BROWSED', 'INCOMPLETE', 'COMPLETED']);
  var COMPLETION_STATE$1 = __AMD("extensions/adapt-contrib-spoor/js/enums/completionStateEnum", COMPLETION_STATE);

  const SUCCESS_STATUS = ENUM(['UNKNOWN', 'PASSED', 'FAILED']);
  var SUCCESS_STATE = __AMD("extensions/adapt-contrib-spoor/js/enums/successStateEnum", SUCCESS_STATUS);
  ...
  var Connection = __AMD("extensions/adapt-contrib-spoor/js/scorm/Connection", class Connection {
    constructor({
      _isEnabled = true,
      _silentRetryLimit = 2,
    ...
    onConnectionSuccess() {
      if (this._isDisconnected) {
        this._scorm.commit();
        if (!this._isSilentDisconnection) Adapt$1.trigger('tracking:connectionSuccess');
      }
      this._isInProgress = false;
      this._isDisconnected = false;
      this.reset();
  ...
  class ScormWrapper {
    ...
    setIncomplete() {
      const options = {
        shouldCommit: this.commitOnStatusChange || this.commitOnAnyChange
      };
      this.setValue(this.isSCORM2004() ? 'cmi.completion_status' : 'cmi.core.lesson_status', COMPLETION_STATE$1.INCOMPLETE.asLowerCase, options);
    }
    setCompleted() {
      const options = {
        shouldCommit: this.commitOnStatusChange || this.commitOnAnyChange
      };
      this.setValue(this.isSCORM2004() ? 'cmi.completion_status' : 'cmi.core.lesson_status', COMPLETION_STATE$1.COMPLETED.asLowerCase, options);
    }
    setPassed() {
      const options = {
        shouldCommit: this.commitOnStatusChange || this.commitOnAnyChange
      ...
      if (this.isSCORM2004()) {
        this.setValue('cmi.completion_status', COMPLETION_STATE$1.COMPLETED.asLowerCase, options);
    ...
    setFailed() {
      ...
      if (this.isSCORM2004()) {
        ...
        if (this.setCompletedWhenFailed) this.setValue('cmi.completion_status', COMPLETION_STATE$1.COMPLETED.asLowerCase, options);
        return;
      }
    ...
    getStatus() {
      ...
      this.handleDataError(new ScormError$1(SERVER_STATUS_UNSUPPORTED, {
        status
      }));
      return null;
    }
    setStatus(status) {
      switch (status.toLowerCase()) {
        case COMPLETION_STATE$1.INCOMPLETE.asLowerCase:
          this.setIncomplete();
          break;
        case COMPLETION_STATE$1.COMPLETED.asLowerCase:
          this.setCompleted();
          break;
        ...
        case SUCCESS_STATE.FAILED.asLowerCase:
          ...
          break;
        default:
          this.handleDataError(new ScormError$1(CLIENT_STATUS_UNSUPPORTED, {
            status
          }));
      }
    }
    ...
    commit() {
      ...
      if (this.scorm.save()) {
        ...
        Adapt$1.trigger('spoor:commit', this);
        return;
      }
      ...
      this.handleDataError(new ScormError$1(CLIENT_COULD_NOT_COMMIT, {
        errorCode,
    ...
    finish() {
      ...
      const errorCode = this.scorm.debug.getCode();
      this.handleFinishError(new ScormError$1(CLIENT_COULD_NOT_FINISH, {
        errorCode,
        errorInfo: this.scorm.debug.getInfo(errorCode),
        diagnosticInfo: this.scorm.debug.getDiagnosticInfo(errorCode)
      }));
    ...
    getValue(property) {
      ...
      switch (errorCode) {
        ...
        case 403:
          ...
          break;
        default:
          this.handleDataError(new ScormError$1(CLIENT_COULD_NOT_GET_PROPERTY, {
            property,
            errorCode,
            errorInfo: this.scorm.debug.getInfo(errorCode),
            diagnosticInfo: this.scorm.debug.getDiagnosticInfo(errorCode)
    ...
    setValue(property, value, {
      shouldCommit = this.commitOnAnyChange
    } = {}) {
      ...
      if (success) {
        ...
        const errorCode = this.scorm.debug.getCode();
        if (errorCode !== 0) {
          this.handleDataError(new ScormError$1(CLIENT_COULD_NOT_SET_PROPERTY, {
            property,
            value,
            errorCode,
            errorInfo: this.scorm.debug.getInfo(errorCode),
    ...
    async handleInitializeError() {
      if (!data$1.isReady) await data$1.whenReady();
      Adapt$1.trigger('tracking:initializeError');
      this.handleError(new ScormError$1(CLIENT_COULD_NOT_CONNECT));
    }
    async handleConnectionError(callback = null) {
      if (!data$1.isReady) await data$1.whenReady();
      Adapt$1.trigger('tracking:connectionError', callback);
      this.handleError(new ScormError$1(CLIENT_NOT_CONNECTED));
    }
    async handleDataError(error) {
      if (!data$1.isReady) await data$1.whenReady();
      if (!this.isUnsupportedErrorCode(error.data.errorCode)) Adapt$1.trigger('tracking:dataError');
      this.handleError(error);
    }
    async handleFinishError(error) {
      if (!data$1.isReady) await data$1.whenReady();
      Adapt$1.trigger('tracking:terminationError');
      this.handleError(error);
    }
    async handleError(error) {
      ...
      const config = Adapt$1.course.get('_spoor');
      const messages = Object.assign({}, ScormError$1.defaultMessages, config && config._messages);
    ...
    recordObjectiveStatus(id, completionStatus, successStatus = SUCCESS_STATE.UNKNOWN.asLowerCase) {
      ...
      if (completionStatus === COMPLETION_STATE$1.COMPLETED.asLowerCase && successStatus !== SUCCESS_STATE.UNKNOWN.asLowerCase) completionStatus = successStatus;
    ...
    isValidCompletionStatus(status) {
      ...
      if (this.isSCORM2004()) {
        switch (status) {
          case COMPLETION_STATE$1.UNKNOWN.asLowerCase:
          case COMPLETION_STATE$1.NOTATTEMPTED.asLowerCase:
          case COMPLETION_STATE$1.NOT_ATTEMPTED.asLowerCase:
          case COMPLETION_STATE$1.INCOMPLETE.asLowerCase:
          case COMPLETION_STATE$1.COMPLETED.asLowerCase:
            return true;
        }
        ...
        switch (status) {
          case COMPLETION_STATE$1.NOTATTEMPTED.asLowerCase:
          case COMPLETION_STATE$1.BROWSED.asLowerCase:
          case COMPLETION_STATE$1.INCOMPLETE.asLowerCase:
          case COMPLETION_STATE$1.COMPLETED.asLowerCase:
          case SUCCESS_STATE.PASSED.asLowerCase:
          case SUCCESS_STATE.FAILED.asLowerCase:
    ...
    getExitState() {
      ...
      const isIncomplete = completionStatus === COMPLETION_STATE$1.INCOMPLETE.asLowerCase || completionStatus === COMPLETION_STATE$1.UNKNOWN.asLowerCase;
  ...
  class LengthAndValueBinary {
    constructor(parent, name, bitSizes) {
      ...
      this.maxValues = ___default["default"].flatten(this.bitSizes).map(value => Math.pow(2, value) - 1);
  ...
  const integerTypes = [new FixedIntegerType({
    name: 'uint3',
    binaryType: '1110',
    valueBinaryLength: 3
  ...
  const decimalTypes = [new VariableDecimalType({
    name: 'sdec15',
    binaryType: '111101',
    minValue: -255.99,
    maxValue: 255.99,
  ...
  function findValueTypeFromValue(value) {
    ...
    const valueType = isFloat ? decimalTypes.find(isValueInValueTypeRange) : integerTypes.find(isValueInValueTypeRange);
    if (!valueType) {
  ...
  function findValueTypeFromValues(values) {
    ...
    const esTypes = values.map(value => {
      const valueType = findValueTypeFromValue(value);
    ...
    const valueType = isFloat ? decimalTypes.find(isRangeInValueTypeRange) : integerTypes.find(isRangeInValueTypeRange);
    if (!valueType) {
  ...
  class Converter {
    ...
    valueToBinary(value, typeName = null, logStats = null) {
      ...
      value = convertValuesToNumbers(value);
      const valueType = hasInitialType ? findValueTypeFromName(typeName) : findValueTypeFromValue(value);
      if (!valueType) {
  ...
  var ComponentSerializer = __AMD("extensions/adapt-contrib-spoor/js/serializers/ComponentSerializer", class ComponentSerializer extends Backbone.Controller {
    ...
    async serialize(shouldStoreResponses, shouldStoreAttempts) {
      if (shouldStoreAttempts && !shouldStoreResponses) {
        logging$1.warnOnce('SPOOR configuration error, cannot use \'_shouldStoreAttempts\' without \'_shouldStoreResponses\'');
      }
      const states = [];
      data$1.each(model => {
        ...
        let components = isContainer ? model.findDescendantModels('component') : [model];
        ...
        components.forEach((component, index) => {
          ...
          if (!shouldStoreResponses) {
            const state = [[trackingId, index], [component.get('_isComplete')]];
            states.push(state);
            return;
          }
          let modelState = null;
          ...
          const state = [[trackingId, index], [hasUserAnswer, isUserAnswerArray, hasAttemptStates], modelState];
          states.push(state);
        });
      });
    ...
    deserialize(binary) {
      const trackingIdMap = data$1.toArray().reduce((trackingIdMap, model) => {
        const trackingId = model.get('_trackingId');
      ...
      states.forEach(state => {
        ...
        if (!component) {
          logging$1.warn(`SPOOR could not restore tracking id: ${trackingId}, index: ${index}`);
          return;
        }
        if (!shouldStoreResponses) {
  ...
  var StatefulSession = __AMD("extensions/adapt-contrib-spoor/js/adapt-stateful-session", class StatefulSession extends Backbone.Controller {
    initialize() {
    ...
    beginSession() {
      ...
      this._trackingIdType = Adapt$1.build.get('trackingIdType') || 'block';
      if (window.location.search.indexOf('nolmserrors') !== -1) {
        this.scorm.suppressErrors = true;
      }
      const config = Adapt$1.spoor.config;
      if (!config) return;
    ...
    setupLearnerInfo() {
      ...
      Object.assign(globals._learnerInfo, offlineStorage$1.get('learnerinfo'));
    }
    restoreSessionState() {
      const sessionPairs = offlineStorage$1.get();
    ...
    setupEventListeners() {
      ...
      this.listenTo(Adapt$1.components, 'change:_isComplete', this.debouncedSaveSession);
      this.listenTo(Adapt$1.contentObjects, 'change:_isComplete', this.onContentObjectCompleteChange);
      this.listenTo(Adapt$1.course, 'change:_isComplete', this.debouncedSaveSession);
      if (this._shouldStoreResponses) {
        this.listenTo(data$1, 'change:_isSubmitted change:_userAnswer', this.debouncedSaveSession);
      }
      this.listenTo(Adapt$1, {
        'app:dataReady': this.restoreSession,
        'adapt:start': this.onAdaptStart,
      ...
      const shouldCommitOnVisibilityChange = !advancedSettings || advancedSettings._commitOnVisibilityChangeHidden !== false;
    ...
    async saveSessionState() {
      ...
      const courseState = SCORMSuspendData.serialize([Boolean(Adapt$1.course.get('_isComplete')), Boolean(Adapt$1.course.get('_isAssessmentPassed'))]);
      const componentStates = await this._componentSerializer?.serialize(this._shouldStoreResponses, this._shouldStoreAttempts);
      const sessionPairs = {
        c: courseState,
        q: componentStates
      };
      offlineStorage$1.set(sessionPairs);
    ...
    printCompletionInformation(suspendData) {
      ...
      const trackingIdModels = data$1.filter(model => model.get('_type') === this._trackingIdType && model.has('_trackingId'));
      const trackingIds = trackingIdModels.map(model => model.get('_trackingId'));
      if (!trackingIds.length) {
        logging$1.info(`course._isComplete: ${courseComplete}, course._isAssessmentPassed: ${assessmentPassed}, ${this._trackingIdType} completion: no tracking ids found`);
        return;
      }
      ...
      logging$1.info(`course._isComplete: ${courseComplete}, course._isAssessmentPassed: ${assessmentPassed}, ${this._trackingIdType} completion: ${completionString}`);
    }
    initializeContentObjectives() {
      if (!this.shouldRecordObjectives) return;
      Adapt$1.contentObjects.forEach(model => {
        if (model.isTypeGroup('course')) return;
        const id = model.get('_id');
        const description = model.get('title') || model.get('displayTitle');
        offlineStorage$1.set('objectiveDescription', id, description);
        if (model.get('_isVisited')) return;
        const completionStatus = COMPLETION_STATE$3.NOTATTEMPTED.asLowerCase;
        offlineStorage$1.set('objectiveStatus', id, completionStatus);
      });
    }
    onAdaptStart() {
      this.setupEventListeners();
    ...
    onLanguageChanged() {
      this.stopListening(Adapt$1.contentObjects, 'change:_isComplete', this.onContentObjectCompleteChange);
      const config = Adapt$1.spoor.config;
      if (config?._reporting?._resetStatusOnLanguageChange !== true) return;
      const completionStatus = COMPLETION_STATE$3.INCOMPLETE.asLowerCase;
      offlineStorage$1.set('status', completionStatus);
    }
    onVisibilityChange() {
    ...
    onQuestionRecordInteraction(view) {
      ...
      const modelId = model.get('_id');
      const id = this._uniqueInteractionIds ? `${this.scorm.getInteractionCount()}-${modelId}` : modelId;
      ...
      const objectiveIds = Adapt$1?.scoring?.getSubsetsByModelId(modelId).filter(set => set.type !== 'adapt').map(({
        id
      }) => id);
      const description = model.get('body');
      offlineStorage$1.set('interaction', id, response, result, latency, responseType, correctResponsesPattern, objectiveIds, description);
    }
    onContentObjectCompleteChange(model) {
      if (!this.shouldRecordObjectives) return;
      if (model.isTypeGroup('course')) return;
      const id = model.get('_id');
      const completionStatus = (model.get('_isComplete') ? COMPLETION_STATE$3.COMPLETED : COMPLETION_STATE$3.INCOMPLETE).asLowerCase;
      offlineStorage$1.set('objectiveStatus', id, completionStatus);
    }
    onTrackingComplete(completionData) {
      ...
      switch (completionData.status) {
        case COMPLETION_STATE$3.COMPLETED:
        case COMPLETION_STATE$3.PASSED:
          {
            if (!config?._reporting?._onTrackingCriteriaMet) {
              logging$1.warn(`No value defined for '_onTrackingCriteriaMet', so defaulting to '${completionStatus}'`);
            } else {
              completionStatus = config._reporting._onTrackingCriteriaMet;
            }
            break;
          }
        case COMPLETION_STATE$3.FAILED:
          {
            if (!config?._reporting?._onAssessmentFailure) {
              logging$1.warn(`No value defined for '_onAssessmentFailure', so defaulting to '${completionStatus}'`);
            } else {
  ...
  var OfflineStorage = __AMD("extensions/adapt-contrib-spoor/js/adapt-offlineStorage-scorm", class OfflineStorageScorm extends Backbone.Controller {
    initialize(statefulSession) {
      this.offlineStorage = offlineStorage$1;
      this.scorm = ScormWrapper$1.getInstance();
      this.statefulSession = statefulSession;
      this.temporaryStore = {};
      this.suspendDataStore = {};
      this.suspendDataRestored = false;
      offlineStorage$1.initialize(this);
    }
    save() {
    ...
    useTemporaryStore() {
      const cfg = Adapt$1.config.get('_spoor');
  ...
  const shouldStart = Object.prototype.hasOwnProperty.call(window, 'ISCOOKIELMS') && window.ISCOOKIELMS !== null;
  const isStoringData = window.ISCOOKIELMS === true;
  ...
  function createResetButton() {
    ...
    $button.on('click', e => {
      if (!e.shiftKey) {
        Cookies__default["default"].remove('_spoor');
  ...
  function start() {
    const GenericAPI = {
      ...
      store(force) {
        if (!isStoringData) return;
        if (!force && Cookies__default["default"].get('_spoor') === undefined) return;
        Cookies__default["default"].set('_spoor', this.data);
        if (Cookies__default["default"].get('_spoor').length !== JSON.stringify(this.data).length) postStorageWarning();
      },
      initialize(defaults = {}) {
        if (!isStoringData) {
          this.data = {};
        ...
        this.data = Cookies__default["default"].getJSON('_spoor');
        if (!this.data) {
          this.data = {};
          Object.entries(defaults).forEach(([path, value]) => set(this.data, path, value));
          this.store(true);
  ...
  class Spoor extends Backbone.Controller {
    initialize() {
      ...
      this.scorm = ScormWrapper$1.getInstance();
      this.listenToOnce(Adapt$1, 'offlineStorage:prepare', this._prepare);
    }
    _prepare() {
      this.config = Adapt$1.config.get('_spoor');
      if (!this.isEnabled) {
  ...
  Adapt$1.spoor = new Spoor();
  __AMD("extensions/adapt-contrib-spoor/js/adapt-contrib-spoor", Adapt$1.spoor);

  function _deepDefaults(original, ...defaultObjects) {
  ...
  function getCompletionAttribute(model = null) {
  ...
  function logTrickleState() {
    ...
    if (!trickleConfig?._logState || logging$1._config?._level !== 'debug') return;
  ...
  class TrickleController extends Backbone.Controller {
    ...
    async scroll(fromModel) {
      ...
      if (!scrollToId) {
        logging$1.error(`Cannot scroll to the next id as none was found at id: "${fromModel.get('_id')}" with _scrollTo: "${trickleConfig._scrollTo}". Suggestion: Set _showEndOfPage to false.`);
        return;
      }
  ...
  var TrickleButtonModel = __AMD("extensions/adapt-contrib-trickle/js/TrickleButtonModel", class TrickleButtonModel extends ComponentModel$1 {
    ...
    calculateButtonText() {
      ...
      const text = isDisabled && trickleConfig._button.disabledText ? trickleConfig._button.disabledText : isStart && trickleConfig._button.startText ? trickleConfig._button.startText : isFinal && trickleConfig._button.finalText ? trickleConfig._button.finalText : trickleConfig._button.text;
  ...
  var TriggeredView = __AMD("extensions/adapt-contrib-triggered/js/triggeredView", class TriggeredView extends Backbone.View {
    get template() {
      return 'triggered-button';
    }
    initialize() {
      this.triggeredView = this.constructor.arguments[0].componentView;
      this.triggeredView.model.set('_isVisible', false, {
        pluginName: '_triggered'
      });
      this.listenTo(Adapt$1, 'remove', this.remove);
      this.render();
    }
    ...
    render() {
      ...
      $container.addClass('triggered__hidden').append(this.$el.html(template(data)));
    }
    show(event) {
      event && event.preventDefault();
      ...
      $(`.triggered__button-hide[data-triggered-id='${currentTriggeredId}']`).removeClass('u-display-none');
    ...
    hide(event) {
      ...
      $(`.triggered__button-show[data-triggered-id='${currentTriggeredId}']`).removeClass('u-display-none');
      a11y$1.focusFirst(this.$el, {
        defer: true
      });
    }
  ...
  class Triggered extends Backbone.Controller {
    initialize() {
      this.listenTo(Adapt$1, 'app:dataReady', this.onDataReady);
    }
    onDataReady() {
      if (!Adapt$1.course.get('_triggered')?._isEnabled) {
        return;
      }
      this.listenTo(Adapt$1, {
        remove: this.remove,
  ...
  __AMD("extensions/adapt-contrib-triggered/js/adapt-contrib-triggered", new Triggered());
  ...
  class Tutor extends Backbone.Controller {
    ...
    getTutorType(model) {
      const config = model.get('_tutor')?._isInherited ?? true ? Adapt$1.course.get('_tutor') : model.get('_tutor');
      if (!config) return;
      return TUTOR_TYPE$1(config._type?.toUpperCase());
    }
    onComponentViewPostRender(view) {
      ...
      if (!type) return;
      const shouldShowFeedback = model.get('_canShowFeedback') && model.get('_isSubmitted') && type === TUTOR_TYPE$1.INLINE;
      if (!shouldShowFeedback) return;
      model.setupFeedback();
      Adapt$1.trigger('questionView:showFeedback', view);
    }
    onButtonsViewPostRender(view) {
      ...
      const type = this.getTutorType(model);
      if (type !== TUTOR_TYPE$1.INLINE) return;
  ...
  class PageIncompletePrompt extends Backbone.Controller {
    ...
    onLanguageChanging() {
      ...
      Adapt$1.once('router:page', () => {
        this.isChangingLanguage = false;
      });
    }
    get courseConfig() {
      return Adapt$1.course.get('_pageIncompletePrompt');
    }
    onPageViewReady() {
      this.inPage = true;
      this.pageModel = data$1.findById(location$1._currentId);
    }
    onLeavePage() {
      if (!this.inPopup) return;
      this.inPopup = false;
      this.stopListening(Adapt$1, 'notify:cancelled');
    ...
    onRouterNavigate(routeArguments) {
      ...
      this.href = /#/.test(window.location.href) ? window.location.href : window.location.href + '#';
      const id = routeArguments[0];
      if (id) {
        if (id === location$1._currentId) return;
        const model = data$1.findById(id);
    ...
    showPrompt() {
      ...
      this.listenToOnce(Adapt$1, 'notify:cancelled', this.onLeaveCancel);
      notify$1.prompt(promptObject);
      this.inPopup = true;
    }
    isEnabled() {
      if (!location$1._currentId) return false;
      ...
      switch (location$1._contentType) {
        case 'menu':
        case 'course':
          this.inPage = false;
          return false;
      }
      const pageModel = data$1.findById(location$1._currentId);
    ...
    enableRouterNavigation(value) {
      router$1.model.set('_canNavigate', value, {
        pluginName: '_pageIncompletePrompt'
      });
    }
  }
  __AMD("extensions/adapt-pageIncompletePrompt/js/adapt-pageIncompletePrompt", new PageIncompletePrompt());
  ...
  class BoxMenuView extends MenuView$1 {
    className() {
      ...
      const backgroundImage = backgroundImages?.[`_${device$1.screenSize}`] ?? backgroundImages?._small;
      const textAlignment = this.model.get('_boxMenu')?._menuHeader?._textAlignment;
      return [`${super.className()} boxmenu`, backgroundImage && 'has-bg-image', textAlignment?._title && `title-align-${textAlignment._title}`, textAlignment?._body && `body-align-${textAlignment._body}`, textAlignment?._instruction && `instruction-align-${textAlignment._instruction}`].join(' ');
    }
    initialize() {
      super.initialize();
    }
  ...
  var BoxMenuModel = __AMD("menu/adapt-contrib-boxMenu/js/BoxMenuModel", class BoxMenuModel extends MenuModel$1 {});

  components$1.register('course menu', {
    view: BoxMenuView$1,
    model: BoxMenuModel
  });
  components$1.register('boxMenu', {
    view: BoxMenuView$1,
    model: BoxMenuModel
  });

  class GridMenuItemView extends MenuItemView$1 {
    className() {
  ...
  var GridMenuItemView$1 = __AMD("menu/adapt-gridMenu/js/GridMenuItemView", GridMenuItemView);

  class GridMenuGroupView extends MenuItemView$1 {
    className() {
  ...
  GridMenuGroupView.childView = GridMenuItemView$1;
  GridMenuGroupView.template = 'gridMenuGroup.jsx';
  var GridMenuGroupView$1 = __AMD("menu/adapt-gridMenu/js/GridMenuGroupView", GridMenuGroupView);

  class GridMenuView extends MenuView$1 {
    className() {
      return `${super.className()} gridmenu`;
    }
    initialize() {
      super.initialize();
      this.setStyles();
    ...
    addChildren() {
      ...
      models.forEach(model => {
        ...
        const ChildView = model.get('_type') === 'menu' && model.get('_gridMenu') && model.get('_gridMenu')._renderAsGroup ? GridMenuGroupView$1 : GridMenuItemView$1;
        const $parentContainer = this.$(this.constructor.childContainer);
        const childView = new ChildView({
          model
        });
    ...
    setBackgroundImage() {
      const config = this.model.get('_gridMenu');
  ...
  GridMenuView.template = 'gridMenu.jsx';
  var GridMenuView$1 = __AMD("menu/adapt-gridMenu/js/GridMenuView", GridMenuView);

  components$1.register('course menu', {
    view: GridMenuView$1
  });
  components$1.register('gridMenu', {
    view: GridMenuView$1,
    model: MenuModel$1.extend({})
  });

  var ThemeView = __AMD("theme/adapt-contrib-vanilla/js/themeView", class ThemeView extends Backbone.View {
    className() {}
    initialize() {
      this.setStyles();
      this.listenTo(Adapt$1, {
    ...
    setMinimumHeight() {
      ...
      if (!minimumHeights) return;
      const minimumHeight = minimumHeights[`_${device$1.screenSize}`] ?? minimumHeights._small;
      this.$el.toggleClass('has-min-height', Boolean(minimumHeight)).css('min-height', minimumHeight ? minimumHeight + 'px' : '');
    }
    setResponsiveClasses() {
      const responsiveClasses = this.model.get('_responsiveClasses');
      if (!responsiveClasses) return;
      this.$el.removeClass(Object.values(responsiveClasses)).addClass(responsiveClasses[`_${device$1.screenSize}`]);
    }
    setCustomStyles() {}
    onRemove() {}
  });

  var ThemePageView = __AMD("theme/adapt-contrib-vanilla/js/themePageView", class ThemePageView extends ThemeView {
    className() {}
    setCustomStyles() {
      this.processHeader();
    }
  ...
  var ThemeArticleView = __AMD("theme/adapt-contrib-vanilla/js/themeArticleView", class ThemeArticleView extends ThemeView {
    className() {}
    setCustomStyles() {}
    onRemove() {}
  });

  var ThemeBlockView = __AMD("theme/adapt-contrib-vanilla/js/themeBlockView", class ThemeBlockView extends ThemeView {
    className() {
  ...
  var ThemeComponentView = __AMD("theme/adapt-contrib-vanilla/js/themeComponentView", class ThemeComponentView extends ThemeView {
    className() {}
    setCustomStyles() {}
    onRemove() {}
  });
  ...
  class Theme extends Backbone.Controller {
    ...
    addFavIcon() {
      const theme = Adapt$1.course.get('_vanilla');
  ...
  register("header", function Header(props) {
    ...
    const {
      ...
      classNamePrefixes = [_type && _type.toLowerCase(), _component && _component.toLowerCase(), _extension && _extension.toLowerCase()].filter(Boolean)
    } = props;
    const sizedBody = mobileBody && !device$1.isScreenSizeMin('medium') ? mobileBody : body;
    const sizedInstruction = mobileInstruction && !device$1.isScreenSizeMin('medium') ? mobileInstruction : instruction;
    const _globals = Adapt$1.course.get('_globals');
    const ariaRegion = _globals?._components?.[`_${_component}`]?.ariaRegion ?? _globals?._extensions?.[`_${_extension}`]?.ariaRegion;
    ...
    return React__default["default"].createElement("div", {
      id: `${_id}-header`,
      className: classes([prefixClasses(classNamePrefixes, ['__header']), props.classes])
      ...
      className: prefixClasses(classNamePrefixes, ['__title-inner']) + ' js-heading',
      ref: jsxHeading
  ...
  register("image", function Image(props) {
    const hasMatchingSetting = Object.hasOwn(props, `_${device$1.screenSize}`) || Object.hasOwn(props, device$1.screenSize);
    const screenSize = hasMatchingSetting ? device$1.screenSize : device$1.isScreenSizeMin('medium') ? 'large' : 'small';
    ...
    return React__default["default"].createElement("span", {
      id: props.id,
      className: classes([prefixClasses(props.classNamePrefixes, [`${classNamePrefixSeparator}image-container`]), props.classes, props.attribution && 'has-attribution'])
    }, React__default["default"].createElement("img", {
      className: classes([prefixClasses(props.classNamePrefixes, [`${classNamePrefixSeparator}image`]), props?._srcFocalPoint && `object-position-${props?._srcFocalPoint}`]),
      src: src,
      "aria-label": props.alt ? a11y$1.normalize(props.alt) : null,
      ...
      dangerouslySetInnerHTML: {
        __html: compile(props.attribution, props)
      }
    })));
  });

  register("navButton", function NavButton(props) {
    const {
      text,
      _iconClasses
    } = props;
    return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement("span", {
      className: classes(['icon', _iconClasses]),
      "aria-hidden": "true"
    }), React__default["default"].createElement("span", {
  ...
  register("tooltip", function Tooltip(props) {
    ...
    return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement("div", {
      ...
      dangerouslySetInnerHTML: {
        __html: compile((isDisabled ? disabledText : text) || '', props)
      }
    })));
  });
  ...
  register("cards", function Cards(props) {
    const {
      _hasClickButton,
      _columns,
      _itemMinHeight,
    ...
    return React__default["default"].createElement("div", {
      ...
      className: classes(['cards__item', _isVisited && 'is-visited', _isActive && 'is-active', _isAnimated && 'is-animating', _graphic.src && 'has-image', !_hasClickButton && 'is-clickable', !_hasClickButton && _hasAction && 'has-action', _classes]),
      key: _index,
      "data-index": _index,
      onClick: !_hasClickButton && _hasAction ? onClick : null,
      style: hasColumns && {
        width: `${100 / _columns[screenSize]}%`
      } || null
    }, React__default["default"].createElement("div", {
      className: "cards__item-inner",
      style: hasMinHeight && {
        minHeight: _itemMinHeight + 'px'
      } || null
    }, React__default["default"].createElement("div", {
      className: "cards__item-container"
    }, _graphic.src && React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['cards__item'],
      attributionClassNamePrefixes: ['cards']
    })), title && React__default["default"].createElement("div", {
      className: "cards__item-title"
    }, html(compile(title))), body && React__default["default"].createElement("div", {
      className: "cards__item-body"
    }, html(compile(body)))), React__default["default"].createElement("div", {
      className: "cards__item-toolbar"
    }, React__default["default"].createElement("div", {
      className: "cards__item-description"
    }, description._graphic.src && React__default["default"].createElement(templates.image, _extends({}, description._graphic, {
      classNamePrefixes: ['cards__item-description'],
      attributionClassNamePrefixes: ['cards']
    })), description.title && React__default["default"].createElement("div", {
      className: "cards__item-description-title"
    }, html(compile(description.title))), description.body && React__default["default"].createElement("div", {
      className: "cards__item-description-body"
    }, html(compile(description.body)))), _hasAction && _hasClickButton && React__default["default"].createElement("div", {
      className: "cards__item-actions"
    }, React__default["default"].createElement("button", {
      onClick: _hasClickButton ? onClick : null,
      onKeyDown: onKeyDown,
      className: "btn-text",
      "aria-label": _isActive ? _buttons._close.ariaLabel : _buttons._open.ariaLabel
    }, html(compile(_isActive ? _buttons._close.buttonText : _buttons._open.buttonText)))), React__default["default"].createElement("div", {
      className: "cards__item-state"
    }, React__default["default"].createElement("div", {
      className: "icon"
    }))))))));
  });
  ...
  register("accordion", function Accordion(props) {
    const visited = Adapt$1.course.get('_globals')?._accessibility?._ariaLabels.visited;
    const {
      _id,
      onClick,
      _isCenterAligned
    ...
    return React__default["default"].createElement("div", {
      ...
      return React__default["default"].createElement("div", {
        className: classes(['accordion-item', 'js-accordion-item', _graphic?.src && 'has-image', _graphic?.src && _imageAlignment && `align-image-${_imageAlignment}`, _isCenterAligned && 'is-center-aligned', _classes]),
        key: _index,
        "data-index": _index
      }, React__default["default"].createElement("div", {
        role: "heading",
        "aria-level": a11y$1.ariaLevel({
          id: _id,
          level: 'componentItem'
        })
      }, React__default["default"].createElement("button", {
        id: `${_id}-${index}-accordion-button`,
        className: classes(['accordion-item__btn', 'js-toggle-item', _titleIcon && 'has-title-icon', _isVisited && 'is-visited', _isActive ? 'is-open is-selected' : 'is-closed']),
        onClick: onClick,
        "aria-expanded": _isActive.toString(),
        "aria-controls": `${_id}-${index}-accordion-button-panel`
      }, React__default["default"].createElement("span", {
        className: "accordion-item__btn-inner"
      }, _titleIcon && React__default["default"].createElement("span", {
        className: "accordion-item__title-icon"
      }, React__default["default"].createElement("span", {
        className: classes(['icon', _titleIcon]),
        "aria-hidden": "true"
      })), React__default["default"].createElement("span", {
  ...
  register("assessmentResults", function AssessmentResults(props) {
    const {
      _globals,
      _retry,
      retryFeedback,
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner assessmentresults__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget assessmentresults__widget"
    }, _isRetryEnabled && React__default["default"].createElement("div", {
      className: "component__feedback assessmentresults__feedback"
    }, React__default["default"].createElement("div", {
      className: "component__feedback-inner assessmentresults__feedback-inner"
    }, retryFeedback && React__default["default"].createElement("div", {
      className: "assessmentresults__retry-feedback"
    }, React__default["default"].createElement("div", {
      className: "assessmentresults__retry-feedback-inner",
      dangerouslySetInnerHTML: {
        __html: compile(retryFeedback, props)
      }
    })), React__default["default"].createElement("button", {
      className: "btn-text assessmentresults__retry-btn js-assessment-retry-btn"
    }, React__default["default"].createElement("span", null, _retry.button || _globals._components._assessmentResults.retryText))))));
  });

  register("blank", function Blank(props) {
    return React__default["default"].createElement("div", {
      className: "component__inner blank__inner"
    });
  });
  ...
  register("gmcq", function Gmcq(props) {
    ...
    return React__default["default"].createElement("div", {
      ...
      _shouldBeSelected,
      _graphic
    }, index) => React__default["default"].createElement("div", {
      className: classes([`gmcq-item item-${index}`, _isRound && 'is-round', _shouldShowMarking && _shouldBeSelected ? 'is-correct' : null, _shouldShowMarking && !_shouldBeSelected ? 'is-incorrect' : null]),
      ...
      checked: _isActive,
      "aria-label": !_shouldShowMarking ? `${a11y$1.normalize(altText || text)} ${_graphic?.alt || ''}` : `${_shouldBeSelected ? ariaLabels.correct : ariaLabels.incorrect}, ${_isActive ? ariaLabels.selectedAnswer : ariaLabels.unselectedAnswer}. ${a11y$1.normalize(altText || text)} ${_graphic?.alt || ''}`,
      ...
      className: classes(['gmcq-item__label', 'js-item-label', 'u-no-select', !_isEnabled && 'is-disabled', _isHighlighted && 'is-highlighted', (_isCorrectAnswerShown ? _shouldBeSelected : _isActive) && 'is-selected']),
      "aria-hidden": true,
      ...
      className: "icon"
    }))), text && React__default["default"].createElement("span", {
      ...
      className: "gmcq-item__state gmcq-item__state-correctness"
    }, React__default["default"].createElement("span", {
  ...
  const LinkWrapper = ({
    ...
    className,
    role
  }) => href ? React__default["default"].createElement("a", {
    href: href,
    target: target,
    className: className,
    role: role
  ...
  register("hotgraphic", function Hotgraphic(props) {
    const {
      _graphic,
      _useGraphicsAsPins
    } = props;
    return React__default["default"].createElement("div", {
      className: "component__inner hotgraphic__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: classes(['component__widget hotgraphic__widget', _useGraphicsAsPins ? 'is-tile' : 'is-pin', _graphic?.attribution && 'has-attribution'])
    }, _useGraphicsAsPins && React__default["default"].createElement(templates.hotgraphicLayoutTiles, props), !_useGraphicsAsPins && React__default["default"].createElement(templates.hotgraphicLayoutPins, props)));
  });

  register("hotgraphicLayoutPins", function HotgraphicLayoutPins(props) {
    const globals = Adapt$1.course.get('_globals');
    ...
    return React__default["default"].createElement("div", {
      className: "hotgraphic__pins"
    }, _graphic && React__default["default"].createElement("img", {
    ...
    }), _graphic?.attribution && React__default["default"].createElement("div", {
      className: "component__attribution hotgraphic__attribution"
    }, React__default["default"].createElement("div", {
      className: "component__attribution-inner hotgraphic__attribution-inner",
      dangerouslySetInnerHTML: {
        __html: compile(_graphic.attribution, _graphic)
      }
    })), React__default["default"].createElement("div", {
      className: "hotgraphic__pin-item-container",
      role: "list"
    }, _items.map(({
      _top,
      _left,
      ...
      const itemCount = compile(globals._components?._hotgraphic?.item || '', {
        itemNumber: _index + 1,
        totalItems: _items.length
      });
      const ariaLabel = `${visited}${numbered}${itemTitle}${itemCount}`;
      return React__default["default"].createElement("div", {
        className: "hotgraphic__pin-item",
        role: "listitem",
        key: _index
      }, React__default["default"].createElement("button", {
        className: classes(['btn-icon hotgraphic__pin', `item-${_index}`, _graphic?._classes, _isVisited && 'is-visited', _pin.src && 'has-pin-image', _pin.src && _pin.srcHover && _pin.srcVisited && 'has-pin-image-states', _useNumberedPins && 'is-numbered-pin', _pinOffsetOrigin && 'offset-origin']),
        "data-index": _index,
      ...
      }), _pin.src && React__default["default"].createElement("span", {
        className: classes(['hotgraphic__pin-image-container', `item-${_index}`])
      }, React__default["default"].createElement("img", {
        className: "hotgraphic__pin-image is-default",
        src: _pin.src,
        "aria-hidden": "true"
      }), _pin.srcHover && React__default["default"].createElement("img", {
        className: "hotgraphic__pin-image is-hover",
        src: _pin.srcHover
      }), _pin.srcVisited && React__default["default"].createElement("img", {
        className: "hotgraphic__pin-image is-visited",
        src: _pin.srcVisited
      })), _useNumberedPins && !_isVisited && React__default["default"].createElement("span", {
        className: "hotgraphic__pin-number",
        "aria-hidden": "true"
      }, index + 1), (!_pin.src && !_useNumberedPins || _isVisited) && React__default["default"].createElement("span", {
        className: "icon",
        "aria-hidden": "true"
      })));
    })));
  ...
  register("hotgraphicLayoutTiles", function HotgraphicLayoutTiles(props) {
    const globals = Adapt$1.course.get('_globals');
    const visitedLabel = globals?._accessibility?._ariaLabels.visited;
    const {
      _items,
    ...
    return React__default["default"].createElement("div", {
      className: "hotgraphic__tile-item-container",
      role: "list"
    }, _items.map(({
      _index,
      _graphic,
      ...
      return React__default["default"].createElement("div", {
        className: "hotgraphic__tile-item",
        role: "listitem",
        key: _index
      }, React__default["default"].createElement("button", {
        className: classes(['hotgraphic__tile', `item-${_index}`, _graphic._classes, _isVisited && 'is-visited']),
        "data-index": _index,
        onClick: onPinClicked
      }, React__default["default"].createElement("span", {
        className: "aria-label"
      }, ariaLabel), React__default["default"].createElement("img", {
        className: "hotgraphic__tile-image",
        src: _graphic.src,
        "aria-hidden": "true"
      }), React__default["default"].createElement("div", {
        className: "icon",
  ...
  register("hotgraphicPopup", function HotgraphicPopup(props) {
    const {
      _items,
      _isRound
    } = props;
    React.useEffect(() => {
      ...
      a11y$1.focusFirst(focusElement);
    });
    return React__default["default"].createElement("div", {
      className: "hotgraphic-popup__inner"
    }, _items.map(({
      title,
      body,
      _graphic,
      ...
      className: classes(['hotgraphic-popup__item', _classes, _isRound && 'is-round', _isVisited && 'is-visited', _isActive && 'is-active', _graphic?.src && `align-image-${_imageAlignment || 'right'}`]),
      key: index,
      "data-index": index,
      "aria-hidden": !_isActive ? true : null
    }, (_imageAlignment === 'left' || _imageAlignment === 'top') && React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      ...
      className: "hotgraphic-popup__item-content"
    }, React__default["default"].createElement("div", {
      className: "hotgraphic-popup__item-content-inner"
    }, title && React__default["default"].createElement("div", {
      ...
      className: "hotgraphic-popup__item-body"
    }, React__default["default"].createElement("div", {
      ...
      classNamePrefixes: ['component-item', 'hotgraphic-popup'],
      attributionClassNamePrefixes: ['component', 'hotgraphic-popup']
    })))), React__default["default"].createElement(templates.hotgraphicPopupToolbar, props));
  });

  register("hotgraphicPopupToolbar", function HotgraphicPopupToolbar(props) {
    const a11yConfig = Adapt$1.config.get('_accessibility');
    const ariaLabels = Adapt$1.course.get('_globals')._accessibility._ariaLabels;
    const {
      onCloseClick,
      onControlClick,
    ...
    return React__default["default"].createElement("div", {
      className: classes(['hotgraphic-popup__toolbar', _hidePagination && 'hide-pagination'])
    }, !_hidePagination && React__default["default"].createElement("div", {
      className: classes(['hotgraphic-popup__nav', !shouldEnableBack && 'first', !shouldEnableNext && 'last'])
    }, React__default["default"].createElement("button", {
      className: classes(['btn-icon hotgraphic-popup__controls back', !shouldEnableBack && 'is-disabled']),
      ...
      className: classes(['btn-icon hotgraphic-popup__controls next', !shouldEnableNext && 'is-disabled']),
      ...
      className: "icon",
      "aria-hidden": "true"
    })), a11yConfig._options._isPopupWrapFocusEnabled && React__default["default"].createElement("a", {
      className: "a11y-focusguard a11y-ignore a11y-ignore-focus",
      tabIndex: "0",
      role: "presentation"
    }));
  });
  ...
  register("matching", function Matching(props) {
    ...
    return React__default["default"].createElement("div", {
      ...
      return React__default["default"].createElement("div", {
        ...
        className: "icon",
        "aria-hidden": "true"
      })))), _canShowCorrectness && React__default["default"].createElement("div", {
        key: `answer-${_index}`,
  ...
  register("matchingDropDown", function MatchingDropDown(props) {
    ...
    }) => itemIndex === _itemIndex || itemIndex === -1);
    ...
    return React__default["default"].createElement("div", {
      ...
      disabled: !_isEnabled,
      className: classes(['dropdown__btn js-dropdown-btn', !_isEnabled && 'is-disabled', hasActiveOption && 'is-selected']),
      role: "combobox",
      ...
      return React__default["default"].createElement("li", {
        ...
        className: "dropdown-item__inner js-dropdown-list-item-inner u-no-select",
        dangerouslySetInnerHTML: {
          __html: displayText || compile(text)
        }
      }));
    })));
  });
  ...
  register("mcq", function Mcq(props) {
    ...
    return React__default["default"].createElement("div", {
      ...
      className: classes(['component__widget', 'mcq__widget', !_isEnabled && 'is-disabled', _isInteractionComplete && 'is-complete is-submitted', _isInteractionComplete && !_canShowCorrectness && !_isCorrectAnswerShown && 'show-user-answer', _isInteractionComplete && _canShowModelAnswer && _isCorrectAnswerShown && 'show-correct-answer', _isInteractionComplete && _canShowCorrectness && 'show-correctness', _isCorrect && 'is-correct']),
      role: _isRadio ? 'radiogroup' : 'group',
      "aria-labelledby": ariaQuestion ? null : (displayTitle || body || instruction) && `${_id}-header`,
      "aria-label": ariaQuestion || null
    }, props._items.map(({
      ...
      "aria-disabled": !_isEnabled,
      checked: _isActive,
      "aria-label": !_shouldShowMarking ? a11y$1.normalize(altText || text) : `${_shouldBeSelected ? ariaLabels.correct : ariaLabels.incorrect}, ${_isActive ? ariaLabels.selectedAnswer : ariaLabels.unselectedAnswer}. ${a11y$1.normalize(altText || text)}`,
      "data-adapt-index": _index,
      onKeyDown: onKeyDown,
      onChange: onItemSelect,
      onFocus: onItemFocus,
      ...
      className: classes(['mcq-item__label', 'u-no-select', !_isEnabled && 'is-disabled', _isHighlighted && 'is-highlighted', (_isCorrectAnswerShown ? _shouldBeSelected : _isActive) && 'is-selected']),
      "aria-hidden": true,
      htmlFor: `${_id}-${index}-input`,
      "data-adapt-index": _index
    }, React__default["default"].createElement("span", {
      ...
      className: "mcq-item__state mcq-item__state-correctness"
    }, React__default["default"].createElement("span", {
  ...
  register("narrative", function Narrative(props) {
    const {
    ...
    return React__default["default"].createElement("div", {
      className: classes(['component__inner narrative__inner', _isLargeMode ? 'mode-large' : 'mode-small', _isTextBelowImageResolved && 'items-are-full-width', _isStackedOnMobile && !_isLargeMode && 'is-stacked'])
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: classes(['component__widget narrative__widget', _hasNavigationInTextAreaResolved && 'narrative__text-controls'])
    }, React__default["default"].createElement(templates.narrativeContent, props), React__default["default"].createElement(templates.narrativeStrapline, props), React__default["default"].createElement(templates.narrativeSlideContainer, props), React__default["default"].createElement(templates.narrativeIndicators, props)));
  });

  register("narrativeContent", function Narrative(props) {
    const {
      _id,
      _items,
      _isLargeMode,
    ...
    return React__default["default"].createElement("div", {
      ...
      className: "narrative__content-inner"
    }, _items.map(({
      _index,
      _isActive,
      _isVisited,
      ...
      className: classes(['narrative__content-item', _isActive && 'is-active', _isVisited && 'is-visited']),
      "aria-hidden": !_isActive || null,
      "data-index": _index,
      key: _index
    }, _graphic && _isStackedOnMobile && !_isLargeMode && React__default["default"].createElement("div", {
      className: "narrative__content-image"
    }, React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['narrative__slider-image js-narrative-swipe'],
      attributionClassNamePrefixes: ['component', 'narrative'],
      draggable: "false"
    }))), title && React__default["default"].createElement("div", {
      className: "narrative__content-title"
    }, React__default["default"].createElement("div", {
      ...
      dangerouslySetInnerHTML: {
        __html: compile(title, props)
      }
    })), body && React__default["default"].createElement("div", {
      className: "narrative__content-body"
    }, React__default["default"].createElement("div", {
      className: "narrative__content-body-inner",
      dangerouslySetInnerHTML: {
        __html: compile(body, props)
      }
    })))), React__default["default"].createElement(templates.narrativeControls, props)));
  });

  register("narrativeControls", function NarrativeControls(props) {
    const {
      _items,
      _isLargeMode,
      _isStackedOnMobile,
    ...
    return React__default["default"].createElement("div", {
      className: "narrative__controls-container u-clearfix"
    }, React__default["default"].createElement("button", {
      "data-direction": "left",
      className: classes(['btn-icon narrative__controls narrative__controls-left', !shouldEnableBack && 'is-disabled']),
      "aria-disabled": !shouldEnableBack || null,
      ...
      className: "narrative__indicators"
    }, _items.map(({
      _index,
      _isVisited,
      _isActive
    }) => React__default["default"].createElement("div", {
      className: classes(['narrative__progress', _isVisited && 'is-visited', _isActive && 'is-selected']),
      "data-index": _index,
      key: _index
    }, React__default["default"].createElement("span", {
      className: "icon",
      ...
      className: classes(['btn-icon narrative__controls narrative__controls-right', !shouldEnableNext && 'is-disabled']),
      "aria-disabled": !shouldEnableNext || null,
      onClick: onNavigationClicked
    }, React__default["default"].createElement("span", {
      className: "aria-label",
  ...
  register("narrativeIndicators", function NarrativeIndicators(props) {
    const {
      _items,
      _isLargeMode,
      _isStackedOnMobile
    ...
    return React__default["default"].createElement("div", {
      className: "narrative__indicators narrative__slide-indicators"
    }, _items.map(({
      _index,
      _isVisited,
      _isActive
  ...
  register("narrativeSlideContainer", function NarrativeSlideContainer(props) {
    const {
      _items,
      _isLargeMode,
      _isStackedOnMobile,
    ...
    return React__default["default"].createElement("div", {
      className: "narrative__slide-container"
    }, React__default["default"].createElement("div", {
      ...
      className: classes(['narrative__slider-image-container', _isActive && 'is-active', _isVisited && 'is-visited', _graphic.attribution && 'has-attribution']),
      style: {
        width: `${_itemWidth}%`
      },
      "aria-hidden": !_isActive || null,
  ...
  register("narrativeStrapline", function NarrativeStrapline(props) {
    const {
      _items,
      _isLargeMode,
      _isStackedOnMobile,
    ...
    const globals = Adapt$1.course.get('_globals')._components._narrative;
    return React__default["default"].createElement("div", {
      className: "narrative__strapline"
    }, React__default["default"].createElement("div", {
      className: "narrative__strapline-header"
    }, React__default["default"].createElement("div", {
      ...
      className: classes(['narrative__strapline-btn', _isVisited && 'is-visited']),
      ...
      className: "narrative__strapline-title"
    }, React__default["default"].createElement("span", {
      className: "narrative__strapline-title-inner",
      dangerouslySetInnerHTML: {
        __html: compile(strapline || title || globals.titleStrapline, props)
      }
    })), React__default["default"].createElement("span", {
      className: "btn-icon narrative__strapline-icon"
    }, React__default["default"].createElement("span", {
      className: "icon",
      "aria-hidden": "true"
    })))))));
  ...
  register("slider", function Slider(props) {
    const {
      _id,
      _globals,
      _shouldShowMarking,
    ...
    const sliderNumberSelectionRef = React.useRef(0);
    ...
    const ariaLabels = Adapt$1.course.get('_globals')._accessibility._ariaLabels;
    return React__default["default"].createElement("div", {
      className: "component__inner slider__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: classes(['component__widget slider__widget', !_isEnabled && 'is-disabled', _isInteractionComplete && 'is-complete is-submitted', _isInteractionComplete && !_canShowCorrectness && !_isCorrectAnswerShown && 'show-user-answer', _isInteractionComplete && _canShowModelAnswer && _isCorrectAnswerShown && 'show-correct-answer', _isInteractionComplete && _canShowCorrectness && 'show-correctness', _shouldShowMarking && _isCorrect && 'is-correct', _shouldShowMarking && !_isCorrect && 'is-incorrect']),
      "aria-labelledby": ariaQuestion ? null : (displayTitle || body || instruction) && `${_id}-header`,
      "aria-label": ariaQuestion || null,
      role: "group"
    }, (labelStart || labelEnd) && React__default["default"].createElement("div", {
      className: "slider__label-container js-slider-label-container"
    }, labelStart && React__default["default"].createElement("div", {
      className: "slider__label-start"
    }, React__default["default"].createElement("div", {
      className: "slider__label-start-inner"
    }, React__default["default"].createElement("span", {
      className: "aria-label"
    }, _globals._components._slider.labelStart, " ", scaleStepPrefix, _scaleStart, scaleStepSuffix), labelStart)), labelEnd && React__default["default"].createElement("div", {
      className: "slider__label-end"
    }, React__default["default"].createElement("div", {
      className: "slider__label-end-inner"
    }, React__default["default"].createElement("span", {
      className: "aria-label"
    }, _globals._components._slider.labelEnd, " ", scaleStepPrefix, _scaleEnd, scaleStepSuffix), labelEnd))), React__default["default"].createElement("div", {
      className: "slider__number-container"
    }, _showScale && _showScaleNumbers && _items.map(({
      index,
      value,
      correct
    }) => {
      ...
      }, _shouldShowMarking && _isInteractionComplete && React__default["default"].createElement("span", {
        className: "aria-label"
      }, `${correct ? ariaLabels.correct : ariaLabels.incorrect}, ${selectedValue === value ? ariaLabels.selectedAnswer : ariaLabels.unselectedAnswer}. ${scaleStepPrefix}${value}${scaleStepSuffix}`), React__default["default"].createElement("span", {
        "aria-hidden": "true"
      }, scaleStepPrefix, value, scaleStepSuffix));
    }), React__default["default"].createElement("div", {
      className: "slider__number-model-range js-slider-model-range"
    }, _isCorrectAnswerShown && getCorrectAnswers().map(correctAnswer => {
      return React__default["default"].createElement("div", {
        className: "slider__number-model-answer",
        key: correctAnswer,
        style: {
          left: `${calculatePercentFromIndex(getIndexFromValue(correctAnswer))}%`
        }
      }, _showNumber && `${scaleStepPrefix}${correctAnswer}${scaleStepSuffix}`);
    })), React__default["default"].createElement("div", {
      className: "slider__number-answer"
    }), _showScaleIndicator && React__default["default"].createElement("div", {
      className: "slider__number-selection js-slider-number-selection a11y-ignore",
      "aria-hidden": "true",
      style: {
        left: `${calculatePercentFromIndex(selectedIndex)}%`
      },
      tabIndex: "-1",
      ref: sliderNumberSelectionRef
    }, _showNumber && `${scaleStepPrefix}${_selectedItem.value}${scaleStepSuffix}`)), _canShowCorrectness && React__default["default"].createElement("div", {
      className: "slider__state",
      "aria-hidden": "true"
    }, _items.slice(0).map((item, index) => React__default["default"].createElement("div", {
      className: classes(['slider__icon', _isInteractionComplete && item.correct && 'slider__correct-icon', _isInteractionComplete && !item.correct && 'slider__incorrect-icon']),
      style: {
        left: `${calculatePercentFromIndex(index)}%`
      },
      key: item.value
      ...
      className: "slider__scale-container js-slider-scale"
    }, React__default["default"].createElement("div", {
      className: "slider__scale-notch slider__scale-notch-start"
    }), _showScale && React__default["default"].createElement("div", {
      className: "slider__scale-notch-container js-slider-scale-notch-container"
    }, _items.slice(1).map((item, index) => React__default["default"].createElement("div", {
      ...
      className: "slider__scale-notch slider__scale-notch-end"
    })), React__default["default"].createElement("div", {
      className: classes(['slider__item', _shouldShowMarking && _isCorrect && 'is-correct', _shouldShowMarking && !_isCorrect && 'is-incorrect'])
    }, React__default["default"].createElement("div", {
      className: "slider__item-input-track"
    }, React__default["default"].createElement("div", {
  ...
  register("textinput", function TextInput(props) {
    ...
    return React__default["default"].createElement("div", {
      ...
      className: classes(['component__widget textinput__widget', !_isEnabled && 'is-disabled', _isInteractionComplete && 'is-complete is-submitted show-user-answer', _isInteractionComplete && _canShowCorrectness && 'show-correctness', _isCorrect && 'is-correct']),
      "aria-labelledby": ariaQuestion ? null : (displayTitle || body || instruction) && `${_id}-header`,
      "aria-label": ariaQuestion || null,
      role: "group"
    }, _items.map(({
  ...
  register("counters", function Counters(props) {
    const {
      _columns
    } = props;
    const screenSize = device$1.screenSize;
    ...
    return React__default["default"].createElement("div", {
      ...
      className: classes(['component__widget', 'counters__widget', hasColumns && 'has-columns', hasColumns && `cols-${_columns[screenSize]}`])
    }, props._items.map(({
      title,
      body,
      _graphic,
      ...
      className: classes(['counters__item', _isVisited && 'is-visited', _isActive && 'is-active', _isAnimated && 'is-animating', _graphic.src && 'has-image', _classes]),
      key: _index,
      "data-index": _index,
      style: hasColumns && {
        width: `${100 / _columns[screenSize]}%`
      ...
      className: "counters__item-container"
    }, _graphic.src && React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['counters__item'],
      attributionClassNamePrefixes: ['counters']
    })), React__default["default"].createElement("div", {
      className: "counters__count"
    }, _countStart, " ", counterLabel && React__default["default"].createElement("span", null, html(compile(counterLabel)))), title && React__default["default"].createElement("div", {
      className: "counters__item-title"
    }, html(compile(title))), body && React__default["default"].createElement("div", {
      className: "counters__item-body"
    }, html(compile(body)))), React__default["default"].createElement("div", {
      className: "counters__item-state"
    }, React__default["default"].createElement("div", {
      className: "icon"
    }))))));
  });

  register("flow-items", function FlowItems({
    onClick,
    _items,
    _buttons,
    _isSequential,
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner flowitems__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget flowitems__widget"
    }, React__default["default"].createElement("div", {
      className: "flowitems__container-items",
      role: "list"
    }, _items.map(({
      _isActive,
      _isVisited,
      ...
      _classes,
      _items
    }, _itemIndex) => React__default["default"].createElement("div", {
      className: classes(['flowitems__flow', _isActive && 'is-active', _isVisited && 'is-visited', _classes]),
      "data-index": _index,
      key: _index
    }, React__default["default"].createElement("div", {
      className: "flowitems__flow-container"
    }, React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['component', 'flowitems-flow']
    })), title && React__default["default"].createElement("div", {
      className: "flowitems__flow-title"
    }, html(compile(title))), body && React__default["default"].createElement("div", {
      className: "flowitems__flow-body"
    }, html(compile(body)))), React__default["default"].createElement("div", {
      className: "flowitems__flow-items-container"
    }, _items.map(({
      title,
      body,
      _graphic,
      _classes
    }, _flowIndex) => React__default["default"].createElement("div", {
      className: classes(['flowitems__flow-item', _graphic.src && 'has-image', _classes]),
      key: _flowIndex,
      "data-index": _flowIndex,
      role: "listitem"
    }, React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['component', 'flowitems-item']
    })), title && React__default["default"].createElement("div", {
      className: "flowitems__flow-item-title"
    }, html(compile(title))), body && React__default["default"].createElement("div", {
      className: "flowitems__flow-item-body"
    }, html(compile(body))))))))), React__default["default"].createElement("div", {
      className: "flowitems__footer"
    }, React__default["default"].createElement("div", {
      className: "flowitems__progress"
    }, _itemActive >= 0 && React__default["default"].createElement("div", {
      className: "flowitems__progress-current"
    }, _itemActive + 1), React__default["default"].createElement("div", {
      className: "flowitems__progress-sep"
    }, "/"), React__default["default"].createElement("div", {
      className: "flowitems__progress-total"
    }, _totalItems)), _buttons && React__default["default"].createElement("div", {
      className: "flowitems__actions"
    }, _buttons._next && React__default["default"].createElement("button", {
      onClick: onClick,
      disabled: _itemActive === _totalItems - 1,
      className: classes(['btn-text', 'flowitems__next', _itemActive === _totalItems - 1 ? 'is-disabled' : '']),
      "aria-label": _buttons._next.ariaLabel
    }, React__default["default"].createElement("div", {
      className: classes(['icon', _buttons._next._icon._classes])
    }, _buttons._next._icon.src && React__default["default"].createElement("img", {
      src: _buttons._next._icon.src,
      alt: _buttons._next._icon.alt
    })), _buttons._next.text && React__default["default"].createElement("div", {
      className: "flowitems__next-text"
    }, html(compile(_buttons._next.text)))), !_isSequential && _buttons._end && React__default["default"].createElement("button", {
      onClick: onClick,
      disabled: _itemActive < _totalItems - 1,
      className: classes(['btn-text', 'flowitems__end', _itemActive < _totalItems - 1 ? 'is-disabled' : '']),
      "aria-label": _buttons._end.ariaLabel
    }, React__default["default"].createElement("div", {
      className: classes(['icon', _buttons._end._icon._classes])
    }, _buttons._end._icon.src && React__default["default"].createElement("img", {
      src: _buttons._end._icon.src,
      alt: _buttons._end._icon.alt
    })), _buttons._end.text && React__default["default"].createElement("div", {
      className: "flowitems__end-text"
    }, html(compile(_buttons._end.text))))))));
  });

  register("list", function List({
    _columns,
    _orderedList,
    _items,
    ...props
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner list__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget list__widget"
    }, React__default["default"].createElement("div", {
      className: classes(['list__container', hasColumns && 'has-columns', _orderedList && 'is-ordered-list']),
      role: "list"
    }, _items.map(({
      _isActive,
      title,
      ...
      key: index,
      className: classes(['list-item', _isActive && 'is-animating', _graphic?.src && 'has-image', title && 'has-title', body && 'has-body', _classes]),
      role: "listitem",
      style: hasColumns && device$1.isScreenSizeMin('medium') && {
        width: `${100 / _columns}%`
      } || null
    }, React__default["default"].createElement("div", {
      className: "list-item__inner"
    }, !_graphic?.src && React__default["default"].createElement("div", {
      ...
      className: "list-item__title-inner",
      dangerouslySetInnerHTML: {
        __html: compile(title, props)
      }
    })), body && React__default["default"].createElement("div", {
      ...
      className: "list-item__body-inner",
      dangerouslySetInnerHTML: {
        __html: compile(body, props)
      }
    })))))))), bodyAfter && React__default["default"].createElement("div", {
      className: "component__body-after list__body-after"
    }, React__default["default"].createElement("div", {
      className: "component__body-after-inner list__body-after-inner",
      dangerouslySetInnerHTML: {
        __html: compile(bodyAfter, props)
      }
    })));
  });

  register("memory", function Memory(props) {
    const {
      _items,
      setActiveItem,
      resetActiveItems,
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner memory__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget memory__widget"
    }, React__default["default"].createElement("div", {
      className: "memory__items"
    }, _items.map(({
      _graphic,
      type,
      _classes,
      ...
      return React__default["default"].createElement("button", {
        ...
        className: classes(['memory-item', 'js-memory-item', _graphic?.src && 'has-image', _isActive && 'is-active is-locked', _isVisited && 'is-visited is-disabled', _classes]),
        key: _index,
        "data-index": _index,
        disabled: _isVisited || _isActive
      }, React__default["default"].createElement("span", {
        className: "memory-item__btn-inner"
      }, React__default["default"].createElement("span", {
        className: "memory-item__icon"
      }, React__default["default"].createElement("span", {
        className: "icon",
        "aria-hidden": "true"
      })), React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      ...
      className: "component__footer memory__footer"
    }, !_isComplete && React__default["default"].createElement("div", {
      className: "memory__found"
    }, React__default["default"].createElement("span", {
      className: "memory__found-label",
      dangerouslySetInnerHTML: {
        __html: compile(status, {
          guesses: guesses.length / 2,
          total: _items.length / 2
        })
      }
    })), _isComplete && feedback && React__default["default"].createElement("div", {
      className: "memory__feedback"
    }, React__default["default"].createElement("div", {
      className: "memory__feedback-inner",
      dangerouslySetInnerHTML: {
        __html: compile(feedback)
      }
    }))));
  });

  register("slider-items", function slideritems(props) {
    const {
      onSliderChange,
      _id,
      _items,
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner slideritems__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget slideritems__widget"
    }, React__default["default"].createElement("div", {
      className: "slideritems__slider"
    }, React__default["default"].createElement("label", {
      htmlFor: `slider-input-${_id}`,
      className: "slideritems__label"
    }, _label), React__default["default"].createElement("input", {
      id: `slider-input-${_id}`,
    ...
    }), React__default["default"].createElement("datalist", {
      id: `slider-input-data-${_id}`
    }, _items.map(({
      _index,
      title
    }, index) => React__default["default"].createElement("option", {
      className: _activeIndex === _index ? 'is-active' : '',
      key: index,
      value: _index
    }, ' ', title)))), React__default["default"].createElement("div", {
      className: "slideritems__container-items",
      role: "list"
    }, _items.map(({
      title,
      body,
      ...
      _isVisited,
      _classes
    }, _index) => React__default["default"].createElement("div", {
      className: classes(['slideritems__item', _graphic.src && 'has-image', _isActive && 'is-active', _isVisited && 'is-visited', _classes]),
      key: _index,
      "data-index": _index,
      role: "listitem"
    }, title && React__default["default"].createElement("div", {
      className: "slideritems__item-title"
    }, html(compile(title))), body && React__default["default"].createElement("div", {
      className: "slideritems__item-body"
    }, html(compile(body))), React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['component', 'slideritems']
    })))))));
  });

  register("tabs", function Tabs(props) {
    const {
      _id,
      _items,
      _minHeight,
    ...
    return React__default["default"].createElement("div", {
      className: "tabs__inner component__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "tabs__widget component__widget clearfix"
    }, React__default["default"].createElement("div", {
      className: "tabs__nav"
    }, React__default["default"].createElement("div", {
      className: "tabs__nav-inner",
      role: "tablist"
    }, _items.map(({
      tabTitle,
      _isActive,
      _index,
      _isVisited
    }, index) => React__default["default"].createElement("button", {
      key: _index,
      role: "tab",
      ...
      className: classes(['tabs__nav-item-btn', 'js-tabs-nav-item-btn-click', _isVisited && 'is-visited', _isActive && 'is-selected']),
      tabIndex: _isActive ? 0 : -1,
      ...
      id: `${_id}-${index}-tabtitle`,
      className: "tabs__nav-item-btn-inner"
    }, tabTitle))))), React__default["default"].createElement("div", {
      ...
      tabIndex: _isActive ? 0 : -1,
      "data-index": _index,
      className: classes(['tabs__content-item', `item-${index}`, _classes, _isActive && 'is-active'])
    }, React__default["default"].createElement("div", {
      className: "tabs__content-item-inner"
    }, title && React__default["default"].createElement("div", {
      className: "tabs__content-item-title"
    }, React__default["default"].createElement("div", {
      className: "tabs__content-item-title-inner",
      role: "heading",
      "aria-level": a11y$1.ariaLevel({
        id: _id,
        level: 'componentItem'
      ...
      className: "tabs__content-item-body"
    }, React__default["default"].createElement("div", {
      className: "tabs__content-item-body-inner",
      dangerouslySetInnerHTML: {
        __html: compile(body)
      }
    })), React__default["default"].createElement(templates.image, _extends({}, _graphic, {
  ...
  register("talk", function Talk(props) {
    const {
      el,
      _items
    } = props;
    ...
    const useObserver = function (options) {
      ...
      const observer = hasObserver ? React.useRef(new IntersectionObserver(observedEntries => {
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner talk__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget talk__widget"
    }, React__default["default"].createElement("div", {
      className: "talk__container-items",
      role: "feed"
    }, _items.map(({
      _character,
      _graphic,
      ...
      key: _index,
      role: "article",
      className: classes(['talk__chat', `talk__chat-character-${_character._index}`, !hasObserver && 'is-visited', _classes]),
      ...
      className: classes(['talk__chat-status'])
    }), React__default["default"].createElement("div", {
      className: "talk__chat-container",
      id: `talk__chat-container-${_index}`
    }, React__default["default"].createElement("div", {
      className: classes(['talk__chat__character', _character._classes])
    }, _character?._graphic && React__default["default"].createElement(templates.image, _extends({}, _character._graphic, {
      classNamePrefixes: ['component', 'talk__character']
    })), _character?.name && React__default["default"].createElement("div", {
      className: "talk__chat__character-title"
    }, html(compile(_character.name)))), React__default["default"].createElement("div", {
      className: "talk__chat__message"
    }, _graphic && React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['component', 'talk__message']
    })), text && React__default["default"].createElement("div", {
      className: "talk__chat__message-text"
    }, html(compile(text))))))))));
  });

  register("typewriter", function Typewriter(props) {
    const {
      words = [],
      typingSpeed = 150,
      erasingSpeed = 100,
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner typewriter__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget typewriter__widget"
    }, React__default["default"].createElement("div", {
      ref: typewriterRef,
      className: "typewriter__container"
    }, React__default["default"].createElement("span", {
      className: "typewriter__text"
    }, currentText))));
  });

  register("vertical-items", function VerticalItems({
    top,
    _items,
    bottom,
    ...props
    ...
    return React__default["default"].createElement("div", {
      className: "component__inner verticalitems__inner"
    }, React__default["default"].createElement(templates.header, props), React__default["default"].createElement("div", {
      className: "component__widget verticalitems__widget"
    }, top && React__default["default"].createElement("div", {
      className: "verticalitems__top u-clearfix"
    }, React__default["default"].createElement("div", {
      className: "verticalitems__top-inner"
    }, html(compile(top)))), React__default["default"].createElement("div", {
      className: "verticalitems__container",
      role: "list"
    }, _items.map(({
      title,
      body,
      ...
      className: classes(['verticalitems__item', 'u-clearfix', _isVisited && 'is-visited', _graphic.src && 'has-image', _classes]),
      key: _index,
      "data-index": _index,
      role: "listitem"
    }, React__default["default"].createElement("div", {
      className: classes(['verticalitems__item-icon', _isVisited ? 'anim' : 'is-hidden'])
    }, React__default["default"].createElement("div", {
      className: classes(['icon', _icon._classes])
    }, _icon.src && React__default["default"].createElement("img", {
      src: _icon.src,
      alt: _icon.alt
    }))), React__default["default"].createElement("div", {
      className: classes(['verticalitems__item__content', _isVisited ? 'anim' : 'is-hidden'])
    }, title && React__default["default"].createElement("div", {
      className: "verticalitems__item__content-title"
    }, html(compile(title))), body && React__default["default"].createElement("div", {
      className: "verticalitems__item__content-body"
    }, html(compile(body))), _graphic.src && React__default["default"].createElement("div", {
      className: "verticalitems__item__content-graphic"
    }, React__default["default"].createElement("img", {
      src: _graphic.src,
      alt: _graphic.alt
    })))))), bottom && React__default["default"].createElement("div", {
      className: "verticalitems__bottom u-clearfix"
    }, React__default["default"].createElement("div", {
      className: "verticalitems__bottom-inner"
    }, html(compile(bottom))))));
  });

  register("bookmarkingResume", function Bookmarking(props) {
    const {
      label,
      ariaLabel,
      isDisabled
    ...
    return React__default["default"].createElement("div", {
      className: "bookmarking-resume__inner"
    }, React__default["default"].createElement("button", {
      type: "button",
      className: classes(['btn-text bookmarking-resume__button js-bookmarking-resume-button', isDisabled && 'is-disabled']),
      "aria-disabled": isDisabled || null,
      "aria-label": ariaLabel
    }, label));
  });

  register("languagePickerNavigationButton", function LanguagePickerNavigationButton(props) {
    const {
      text,
      _iconClasses
    } = props;
    return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement("span", {
      ...
      "aria-hidden": "true",
      dangerouslySetInnerHTML: {
        __html: compile(text, props)
      }
    }));
  });

  register("pageLevelProgress", function PageLevelProgress(props) {
    const {
      _items,
      _item,
      _globals
    ...
    return React__default["default"].createElement("div", {
      className: "pagelevelprogress__inner"
    }, React__default["default"].createElement("span", {
      className: "aria-label"
    }, compile(_globals._extensions._pageLevelProgress.pageLevelProgress)), React__default["default"].createElement("div", {
      role: "list",
      className: "js-children"
    }, _items && _items.map(item => React__default["default"].createElement(templates.pageLevelProgressItem, _extends({}, item, {
      _globals: _globals,
      key: item._id
    }))), _item && React__default["default"].createElement(templates.pageLevelProgressItem, _extends({}, _item, {
      _globals: _globals,
      key: _item._id
    }))));
  });

  register("pageLevelProgressIndicator", function PageLevelProgressIndicator(props) {
    const {
      ariaLabel
    } = props;
    return React__default["default"].createElement("span", {
      className: "pagelevelprogress__indicator"
    }, React__default["default"].createElement("span", {
      className: "pagelevelprogress__indicator-inner"
    }, React__default["default"].createElement("span", {
      className: "pagelevelprogress__indicator-bar"
    }), ariaLabel && React__default["default"].createElement("span", {
      className: "aria-label"
    }, compile(ariaLabel, props))));
  });

  register("pageLevelProgressItem", function PageLevelProgressItem(props) {
    const {
      _globals,
      _isOptional,
      _isLocked,
    ...
    const indicatorSeat = React__default["default"].createRef();
    React.useEffect(() => {
      if (_isOptional) return;
      const model = data$1.findById(_id);
    ...
    return React__default["default"].createElement("div", {
      className: classes(['pagelevelprogress__item drawer__item', `${_type}__indicator`, location$1._currentModel.get('_id') === _id && 'is-current-location']),
      role: "listitem"
    }, React__default["default"].createElement("button", {
      className: classes(['pagelevelprogress__item-btn drawer__item-btn', 'js-indicator js-pagelevelprogress-item-click', `is-${_type}-indicator`, _isComplete && 'is-complete', _isOptional && 'is-optional', _isLocked && 'is-locked', (_isLocked || !_isVisible) && 'is-disabled']),
      ref: indicatorSeat,
      "data-pagelevelprogress-id": _id,
      "aria-label": classes([(_isLocked || !_isVisible) && `${_globals._accessibility._ariaLabels.locked}.`, _isOptional && `${_globals._extensions._pageLevelProgress.optionalContent}.`, !_isOptional && _isComplete && `${_globals._accessibility._ariaLabels.complete}.`, !_isOptional && !_isComplete && `${_globals._accessibility._ariaLabels.incomplete}.`, isCorrect && `${_globals._accessibility._ariaLabels.answeredCorrectly}.`, isPartlyCorrect && `${_globals._accessibility._ariaLabels.answeredPartlyCorrect ?? _globals._accessibility._ariaLabels.answeredIncorrectly}.`, isIncorrect && `${_globals._accessibility._ariaLabels.answeredIncorrectly}.`, compile(a11y$1.normalize(altTitle || title))])
    }, React__default["default"].createElement("span", {
      className: "pagelevelprogress__item-title drawer__item-title"
    }, React__default["default"].createElement("span", {
      className: "pagelevelprogress__item-title-inner drawer__item-title-inner",
      dangerouslySetInnerHTML: {
        __html: compile(altTitle || title, props)
      }
    })), _isOptional && React__default["default"].createElement("span", {
      className: "pagelevelprogress__item-optional"
    }, React__default["default"].createElement("span", {
      className: "pagelevelprogress__item-optional-inner"
    }, _globals._extensions._pageLevelProgress.optionalContent)), React__default["default"].createElement("span", {
      className: "pagelevelprogress__item-icon"
    }, React__default["default"].createElement("span", {
      className: "icon",
      "aria-hidden": "true"
    }))), React__default["default"].createElement("div", {
      className: "pagelevelprogress__item-children"
    }, React__default["default"].createElement("div", {
      role: "list",
      className: "js-children"
    }, _children && _children.map(item => React__default["default"].createElement(PageLevelProgressItem, _extends({}, item, {
      key: item._id,
      _globals: _globals
    }))))));
  });

  register("pageLevelProgressNavigationButton", function PageLevelProgressNavigationButton(props) {
    const {
      text,
      _iconClasses
    } = props;
    return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement("span", {
      ...
      className: "pagelevelprogress__indicator-wrapper"
    }), React__default["default"].createElement("span", {
      className: "nav__btn-label",
      "aria-hidden": "true"
    }, compile(text, props)));
  ...
  register("resources", function Resources(props) {
    const {
      resources,
      resourceTypes,
      _showFilters,
    ...
    const [selectedId, setSelectedId] = React.useState('resources__show-all');
    ...
    return React__default["default"].createElement("div", {
    ...
    }, React__default["default"].createElement(templates.header, props.model), _showFilters && React__default["default"].createElement("div", {
      className: classes(['resources__filter', `has-${_filterColumnCount}-columns`, _filterColumnCount > 4 && 'has-extra-types'])
    }, React__default["default"].createElement("div", {
      className: "resources__filter-inner",
      role: "tablist"
    }, React__default["default"].createElement("div", {
      className: "aria-label",
      "aria-label": _globals._extensions._resources.resources
    }), resourceTypes.map((type, index) => React__default["default"].createElement(templates.resourcesFilterButton, _extends({}, props, {
      key: index,
      resources: resources,
      _filter: type,
      ...
      id: "resources",
      className: "resources__item-container",
      role: "tabpanel",
      ...
      filename,
      _forceDownload
    }, index) => React__default["default"].createElement(templates.resourcesItem, _extends({}, props, {
      key: index,
      _index: index,
      title: title,
  ...
  register("resourcesFilterButton", function ResourcesFilterButton(props) {
    const {
      model,
      onClick,
      selected,
    ...
    return React__default["default"].createElement("button", {
      ...
      className: classes(['resources__filter-btn', selected === _filter && 'is-selected']),
      onClick: onClick,
      "data-filter": _filter,
      "aria-label": ariaLabel,
      role: "tab",
  ...
  register("resourcesItem", function ResourcesItem(props) {
    const {
      _forceDownload,
      _canDownload,
      _isGlobal,
    ...
    return React__default["default"].createElement("div", {
      className: classes(['resources__item drawer__item', `is-${_type}`, _isGlobal && 'is-global', !['all', _type].includes(selectedFilter) && 'u-display-none']),
      role: "listitem"
    }, React__default["default"].createElement("a", {
      href: _link,
      className: "resources__item-btn drawer__item-btn",
      "data-type": _type,
      "data-index": _index,
      ...
      className: "resources__item-title drawer__item-title"
    }, React__default["default"].createElement("div", {
      className: "resources__item-title-inner drawer__item-title-inner",
      dangerouslySetInnerHTML: {
        __html: title
      }
    })), description && React__default["default"].createElement("div", {
      className: "resources__item-body drawer__item-body"
    }, React__default["default"].createElement("div", {
      className: "resources__item-body-inner drawer__item-body-inner",
      dangerouslySetInnerHTML: {
        __html: description
      }
    }))));
  ...
  register("boxMenu", function BoxMenu(props) {
    const {
      displayTitle,
      subtitle,
      body,
    ...
    const _boxMenu = Adapt$1.course.get('_boxMenu');
    ...
    const headerBackgroundImage = headerBackgroundImages?.[`_${device$1.screenSize}`] ?? headerBackgroundImages?._small;
    const headerBackgroundStyles = header?._backgroundStyles || {};
    const headerMinimumHeights = header?._minimumHeights;
    const headerMinimumHeight = headerMinimumHeights?.[`_${device$1.screenSize}`] ?? headerMinimumHeights?._small;
    return React__default["default"].createElement(React__default["default"].Fragment, null, backgroundImages && React__default["default"].createElement("div", {
      ...
      className: "menu__inner boxmenu__inner"
    }, (displayTitle || subtitle || body || instruction) && React__default["default"].createElement("div", {
      className: classes(['menu__header', 'boxmenu__header', headerBackgroundImage && 'has-bg-image', headerMinimumHeight && 'has-min-height']),
      style: headerMinimumHeight ? {
        minHeight: headerMinimumHeight + 'px'
      } : null
    }, headerBackgroundImages && React__default["default"].createElement("div", {
      className: "background",
      "aria-hidden": "true",
      style: {
      ...
      className: "menu__header-inner boxmenu__header-inner"
    }, _graphic?._src && React__default["default"].createElement("div", {
      className: "menu__image-container boxmenu__image-container"
    }, React__default["default"].createElement("img", {
      ...
      className: "menu__header-content boxmenu__header-content"
    }, displayTitle && React__default["default"].createElement("div", {
      className: "menu__title boxmenu__title"
    }, React__default["default"].createElement("div", {
      className: classes(['menu__title-inner', 'boxmenu__title-inner', 'js-heading'])
    })), subtitle && React__default["default"].createElement("div", {
      className: "menu__subtitle boxmenu__subtitle"
    }, React__default["default"].createElement("div", {
      className: "menu__subtitle-inner boxmenu__subtitle-inner",
      dangerouslySetInnerHTML: {
        __html: compile(subtitle)
      }
    })), (body || pageBody) && React__default["default"].createElement("div", {
      className: "menu__body boxmenu__body"
    }, React__default["default"].createElement("div", {
      className: "menu__body-inner boxmenu__body-inner",
      dangerouslySetInnerHTML: {
        __html: compile(pageBody || body)
      }
    })), instruction && React__default["default"].createElement("div", {
      className: "menu__instruction boxmenu__instruction"
    }, React__default["default"].createElement("div", {
      className: "menu__instruction-inner boxmenu__instruction-inner",
      dangerouslySetInnerHTML: {
        __html: compile(instruction)
      }
    }))))), React__default["default"].createElement("div", {
      className: "menu__item-container boxmenu__item-container"
    }, React__default["default"].createElement("div", {
      className: classes(['menu__item-container-inner', 'boxmenu__item-container-inner', 'js-children']),
      role: "list"
    }))));
  });

  register("boxMenuGroup", function BoxMenuGroup(props) {
    const {
      displayTitle,
      body,
      instruction
    ...
    return React__default["default"].createElement("div", {
      className: "menu-group__inner boxmenu-group__inner"
    }, React__default["default"].createElement("div", {
      className: "menu-group__header boxmenu-group__header"
    }, React__default["default"].createElement("div", {
      className: "menu-group__header-inner boxmenu-group__header-inner"
    }, displayTitle && React__default["default"].createElement("div", {
      className: "menu-group__title boxmenu-group__title"
    }, React__default["default"].createElement("div", {
      className: classes(['menu-group__title-inner', 'boxmenu-group__title-inner', 'js-heading']),
      "data-a11y-heading-type": "menuGroup"
    })), body && React__default["default"].createElement("div", {
      className: "menu-group__body boxmenu-group__body"
    }, React__default["default"].createElement("div", {
      ...
      className: "menu-group__instruction boxmenu-group__instruction"
    }, React__default["default"].createElement("div", {
      ...
      className: classes(['menu-group__progress', 'boxmenu-group__progress', 'js-menu-item-progress'])
    }))), React__default["default"].createElement("div", {
      className: "menu-group__container boxmenu-group__container"
    }, React__default["default"].createElement("div", {
      className: classes(['menu-group__item-container-inner', 'boxmenu-group__item-container-inner', 'js-group-children']),
      role: "list"
    })));
  });

  register("boxMenuItem", function BoxMenuItem(props) {
    const {
      displayTitle,
      _graphic,
      body,
    ...
    const _globals = Adapt$1.course.get('_globals');
    const durationLabel = duration ? [_globals?._menu?._boxMenu?.durationLabel.trim(), duration].filter(Boolean).join(' ') : '';
    ...
    const itemCount = compile(_globals?._menu?._boxMenu?.itemCount || '', {
      _nthChild,
      _totalChild
    });
    const ariaLabel = [completion, locked, `${title}.`, `${itemCount}.`, `${optional}`].filter(Boolean).join(' ');
    return React__default["default"].createElement("div", {
      className: "menu-item__inner boxmenu-item__inner"
    }, React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['menu-item', 'boxmenu-item'],
      alt: null,
      attribution: null
    })), React__default["default"].createElement("div", {
      className: "menu-item__details boxmenu-item__details"
    }, React__default["default"].createElement("div", {
      className: "menu-item__details-inner boxmenu-item__details-inner"
    }, displayTitle && React__default["default"].createElement("div", {
      className: "menu-item__title boxmenu-item__title"
    }, React__default["default"].createElement("div", {
      className: classes(['menu-item__title-inner', 'boxmenu-item__title-inner', 'js-heading']),
      "data-a11y-heading-type": "menuItem"
    })), _graphic?.alt && React__default["default"].createElement("span", {
      className: "aria-label",
      dangerouslySetInnerHTML: {
        __html: compile(_graphic.alt)
      }
    }), body && React__default["default"].createElement("div", {
      className: "menu-item__body boxmenu-item__body"
    }, React__default["default"].createElement("div", {
      className: "menu-item__body-inner boxmenu-item__body-inner",
      dangerouslySetInnerHTML: {
        __html: compile(body)
      }
    })), duration && React__default["default"].createElement("div", {
      className: "menu-item__duration boxmenu-item__duration"
    }, React__default["default"].createElement("div", {
      className: "menu-item__duration-inner boxmenu-item__duration-inner",
      dangerouslySetInnerHTML: {
        __html: compile(durationLabel, props)
      }
    })), React__default["default"].createElement("div", {
      className: classes(['menu-item__progress', 'boxmenu-item__progress', 'js-menu-item-progress'])
    }), React__default["default"].createElement("div", {
      className: "menu-item__button-container boxmenu-item__button-container"
    }, React__default["default"].createElement("button", {
      className: classes(['btn-text', 'menu-item__button', 'boxmenu-item__button', 'js-btn-click', _isVisited && 'is-visited', _isLocked && 'is-locked']),
      "aria-label": ariaLabel,
      "aria-disabled": _isLocked ? true : null,
      role: "link"
    }, linkText && React__default["default"].createElement("span", {
      className: "menu-item__button-text boxmenu-item__button-text",
      dangerouslySetInnerHTML: {
        __html: compile(linkText)
      }
    })), React__default["default"].createElement("span", {
      className: "menu-item__status boxmenu-item__status"
    }, React__default["default"].createElement("span", {
      className: "icon",
      "aria-hidden": "true"
    }))))));
  ...
  register("gridMenu", function GridMenu(props) {
    return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement(templates.header, _extends({}, props, {
      classNamePrefixes: ['menu', 'gridmenu']
    })), props._gridMenu?._graphic && React__default["default"].createElement(templates.image, _extends({}, props._gridMenu._graphic, {
      classNamePrefixes: ['gridmenu'],
      attributionClassNamePrefixes: ['gridmenu']
    })), React__default["default"].createElement("div", {
      className: "gridmenu__items-container js-children",
      role: "list"
    }));
  });

  register("gridMenuGroup", function GridMenuItem(props) {
    return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement(templates.header, _extends({}, props, {
      classNamePrefixes: ['menu', 'gridmenu-group']
    })), React__default["default"].createElement("div", {
      className: "gridmenu-group__items-container js-group-children"
    }));
  });

  register("gridMenuItem", function GridMenuItem(props) {
    const globals = Adapt$1.course.get('_globals');
    const {
      title,
      displayTitle,
    ...
    const durationLabel = duration ? globals._menu._gridMenu?.durationLabel ? `${globals._menu._gridMenu?.durationLabel.trim()} ${duration}` : duration : '';
    return React__default["default"].createElement(React__default["default"].Fragment, null, React__default["default"].createElement("button", {
      ...
      role: "link",
      "aria-label": [_isComplete && `${globals._accessibility._ariaLabels.complete}.`, _isVisited && `${globals._accessibility._ariaLabels.visited}.`, linkText, `${title}.`, durationLabel].filter(Boolean).join(' '),
      "aria-disabled": _isLocked ? true : undefined
    }, React__default["default"].createElement(templates.image, _extends({}, _graphic, {
      classNamePrefixes: ['gridmenu-item'],
      attributionClassNamePrefixes: ['gridmenu']
    })), React__default["default"].createElement("div", {
      className: "gridmenu-item__content"
    }, displayTitle && React__default["default"].createElement("div", {
      className: "gridmenu-item__title"
    }, React__default["default"].createElement("div", {
      className: "gridmenu-item__title-inner js-heading",
      "data-a11y-heading-type": "menuItem"
    })), body && React__default["default"].createElement("div", {
      className: "gridmenu-item__body",
      dangerouslySetInnerHTML: {
        __html: compile(body, props)
      }
    }), duration && React__default["default"].createElement("div", {
      className: "gridmenu-item__duration",
      dangerouslySetInnerHTML: {
        __html: compile(durationLabel, props)
      }
    })), React__default["default"].createElement("span", {
      className: "menu-item__status gridmenu-item__status"
    }, React__default["default"].createElement("span", {
      className: "icon",
      "aria-hidden": "true"
    }))), React__default["default"].createElement("div", {
      className: "js-menu-item-progress"
    }));
  });
